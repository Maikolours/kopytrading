...
void OnTick() {
   ...
   double currentPct = (totalDist > 0) ? (dist0 / totalDist) * 100 : 0;
   
   if(currentPct < 5.0 && PositionsTotalBots() > 0) {
   ...
   // --- LÓGICA DE REVERSIÓN (CAMBIO DE TENDENCIA) ---
...
void DrawInteractiveSLLine() {
   ...
   double slPrice = f_p100 - (f_p100 - f_p0) * (currSLPct/100.0);
...
void ProtectPositions() {
   for(int i=PositionsTotal()-1; i>=0; i--) {
      if(posInfo.SelectByIndex(i) && posInfo.Magic()==MagicNumber) {
         ...
         double stpLimit = SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL) * _Point + (10 * _Point);
         ...
         double redLineSL = ObjectGetDouble(0, PNL_TAG+"SL_DRAG", OBJPROP_PRICE);
         
         if(posInfo.PositionType() == POSITION_TYPE_BUY) {
            if(profit > currBE && curSL < openP) newSL = openP + currGarantia;
            if(profit > currTSDist && (bid - openP) > currTS) {
               double trailVal = bid - currTS;
               if(trailVal > newSL) newSL = trailVal;
            }
            if(redLineSL > newSL) newSL = redLineSL;
            // Broker Safety Check
            if(bid - newSL < stpLimit) newSL = bid - stpLimit;
         } else {
            if(profit > currBE && curSL > openP) newSL = openP - currGarantia;
            if(profit > currTSDist && (openP - ask) > currTS) {
               double trailVal = ask + currTS;
               if(trailVal < newSL) newSL = trailVal;
            }
            if(redLineSL < newSL && redLineSL != 0) newSL = redLineSL;
            if(newSL - ask < stpLimit) newSL = ask + stpLimit;
         }
         
         if(MathAbs(newSL - curSL) > _Point) {
            trade.PositionModify(posInfo.Ticket(), NormalizeDouble(newSL, _Digits), 0);
         }
      }
   }
}
...
void CrearHUD() {
   ...
   if(!isMinimized) {
      ...
      ry += 35;
      CrBtn("b_app",h_x+10,ry,280,30,"APLICAR CAMBIO MATRIX",C'30,60,120',clrWhite);
   ...
   if(id==CHARTEVENT_OBJECT_CLICK) {
      ...
      if(sp==PNL_TAG+"v_ma")  { 
         ...
         if(!showEMA) { ChartIndicatorDelete(0, 0, "EMA(20)"); ChartIndicatorDelete(0, 0, "EMA(50)"); }
            ...
            // Configurar Colores Elite (Azul y Rojo)
            long cid = 0; 
