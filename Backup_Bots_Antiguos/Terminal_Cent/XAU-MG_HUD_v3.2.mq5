//+------------------------------------------------------------------+
//| XAU-MG_HUD_v3.2.mq5 - AMETRALLADORA                              |
//| Versión Profesional - Fibonacci + Confluencia + Extensiones      |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, Kopytrading Corp."
#property link      "https://www.kopytrading.com"
#property version   "3.20"
#property description "AMETRALLADORA v3.2 - Bot Profesional"
#property description "Fibonacci OTE | Confluencia | Extensiones | Multi-TF"
...
//+------------------------------------------------------------------+
//| 1. TENDENCIA H1 (EMA 50)                                         |
//+------------------------------------------------------------------+
bool IsH1TrendUp() {
   double emaH1[];
   if(h_ema_h1 == INVALID_HANDLE) return true;
   if(CopyBuffer(h_ema_h1, 0, 0, 1, emaH1) <= 0) return true;
   double price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   return (price > emaH1[0]);
}

//+------------------------------------------------------------------+
//| 2. DETECTAR IMPULSO EN M15 (últimas 10 velas = 2.5 horas)        |
//+------------------------------------------------------------------+
bool DetectImpulse(int &direction, double &startPrice, double &endPrice) {
   ...
   // 6 de 10 velas = 60% de acuerdo → impulso válido
   if(up_count >= 6) {
      direction = 1;    // COMPRA
      startPrice = min_low;
      endPrice = max_high;
...
void CalculateFibonacci() {
   bool h1_up = IsH1TrendUp();
   int impulse_dir;
   double start, end;
   
   if(DetectImpulse(impulse_dir, start, end)) {
      // Verificar que impulso y tendencia H1 van en misma dirección
      if((h1_up && impulse_dir == 1) || (!h1_up && impulse_dir == -1)) {
         fibo_0 = start;
         fibo_100 = end;
         fibo_direction = impulse_dir;
         double range = MathAbs(end - start);
         
         if(impulse_dir == 1) { // COMPRA
            fibo_618 = end - range * user_Fibo_618;
            fibo_786 = end - range * user_Fibo_786;
            tp1 = end + range * user_Fibo_272;      // Extensión 27.2%
            tp2 = end + range * user_Fibo_618_ext;  // Extensión 61.8%
...
//+------------------------------------------------------------------+
//| 4. VERIFICAR CONFLUENCIA (niveles clave)                         |
//+------------------------------------------------------------------+
bool HasConfluence(double price) {
   // 4.1 Números redondos (Oro: 4500, 4550, etc.)
   double round_level = MathRound(price / 50) * 50;
   if(MathAbs(price - round_level) < 5 * _Point) return true;
   
   // 4.2 Soportes/resistencias de H1 (últimas 100 velas)
   double highs[], lows[];
   if(CopyHigh(_Symbol, PERIOD_H1, 0, 100, highs) >= 100 &&
      CopyLow(_Symbol, PERIOD_H1, 0, 100, lows) >= 100) {
      
      for(int i = 0; i < 100; i++) {
         if(MathAbs(price - highs[i]) < 8 * _Point) return true;
         if(MathAbs(price - lows[i]) < 8 * _Point) return true;
      }
   }
...
//+------------------------------------------------------------------+
//| 5. CONFIRMACIÓN CON VELA DE RECHAZO EN M5                        |
//+------------------------------------------------------------------+
bool HasRejectionCandle() {
   MqlRates rates[];
   if(CopyRates(_Symbol, PERIOD_M5, 0, 1, rates) < 1) return false;
   ...
   if(fibo_direction == 1) { // COMPRA: busca martillo (mecha larga abajo)
      return (lower_wick > body * 2 && rates[0].close > rates[0].open);
   } else if(fibo_direction == -1) { // VENTA: busca estrella fugaz (mecha larga arriba)
      return (upper_wick > body * 2 && rates[0].close < rates[0].open);
   }
...
//+------------------------------------------------------------------+
//| ACTUALIZAR INFORMACIÓN DE TENDENCIA PARA HUD                     |
//+------------------------------------------------------------------+
void UpdateTrendInfo() {
   bool h1_up = IsH1TrendUp();
   double emaF[1], emaS[1];
   
   if(CopyBuffer(h_ema20, 0, 0, 1, emaF) > 0 && CopyBuffer(h_ema50, 0, 0, 1, emaS) > 0) {
      if(emaF[0] > emaS[0]) {
   ...
   if(h1_up) {
      currentTrend += " (H1 ALCISTA)";
   } else {
      currentTrend += " (H1 BAJISTA)";
   }
}
...
void DrawFibonacciLevels() {
   ...
   // Zona de entrada (rectángulo semitransparente)
   ...
   // Texto informativo
   string dirText = (fibo_direction == 1) ? "COMPRA" : "VENTA";
   string info = "FIBONACCI | " + dirText + " | Zona: " + DoubleToString(fibo_786, _Digits) + " - " + DoubleToString(fibo_618, _Digits);
   ObjectCreate(0, "FIBO_INFO", OBJ_LABEL, 0, 0, 0);
   ObjectSetInteger(0, "FIBO_INFO", OBJPROP_XDISTANCE, 10);
   ObjectSetInteger(0, "FIBO_INFO", OBJPROP_YDISTANCE, 55);
...
void SmartRecovery() {
   ...
   if(mainType == POSITION_TYPE_BUY) {
      priceInZone = (currentPrice >= fibo_786 && currentPrice <= fibo_618);
   } else {
      priceInZone = (currentPrice <= fibo_786 && currentPrice >= fibo_618);
   }
...
//+------------------------------------------------------------------+
//| ENTRADA CON FIBONACCI + CONFLUENCIA + CONFIRMACIÓN               |
//+------------------------------------------------------------------+
void CheckEntry() {
   if(GetPositionsCount() >= runtime_MaxPosiciones) return;
   ...
   bool inFiboZone = false;
   if(fibo_direction == 1) {
      inFiboZone = (priceNow >= fibo_786 && priceNow <= fibo_618);
   } else {
      inFiboZone = (priceNow <= fibo_786 && priceNow >= fibo_618);
   }
   ...
   if(!HasConfluence(priceNow)) {
      botStatus = "EN ZONA FIBO, SIN CONFLUENCIA";
      return;
   }
   
   // Verificar vela de rechazo en M5
   if(!HasRejectionCandle()) {
      botStatus = "ZONA FIBO+CONFLUENCIA, ESPERANDO VELA";
      return;
   }
   ...
   // --- TODAS LAS CONDICIONES CUMPLIDAS: EJECUTAR OPERACIÓN ---
   if(fibo_direction == 1) {
...
void CrearHUD() {
   ...
   int x = 10, y = 10, w = 480, h = 800;
   
   CrRect("bg", x, y, w, h, C'15,15,35', C'60,60,120', 2);
   
   string title = "⚡ AMETRALLADORA v3.2 | " + (isCentAccount ? "CENT" : "REAL");
   ...
   int row = innerY, colLabel = 0, colValue = 160, colEdit = innerW - 100;
   ...
   // Parámetros con casillas de texto
   ...
   CrLabel("metaL", innerX+colLabel, row, "🔄 META CICLO:", C'150,150,200', 10);
