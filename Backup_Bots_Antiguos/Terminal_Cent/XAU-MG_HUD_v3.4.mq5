//+------------------------------------------------------------------+
//| XAU-MG_HUD_v3.4.mq5 - AMETRALLADORA                              |
//| Versión Unificada - Análisis Profesional de Velas                |
//| Martillo | Envolvente | Doji | Estrella Fugaz                    |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, Kopytrading Corp."
#property link      "https://www.kopytrading.com"
#property version   "3.40"
#property description "AMETRALLADORA v3.4 - Análisis Profesional de Velas"
#property description "Martillo | Envolvente | Doji | Estrella Fugaz"
#property description "Fibonacci OTE | Confluencia (recomendada no obligatoria)"
...
input bool      user_RequireConfluence = false;  // Si es true, exige confluencia
...
int            lastCandleSignal = 0;  // 1=compra, -1=venta, 0=neutral
...
//+------------------------------------------------------------------+
//| 4. ANÁLISIS PROFESIONAL DE VELAS (M5)                            |
//+------------------------------------------------------------------+
void AnalyzeCandlestick() {
   ...
   // 4.1 MARTILLO (señal de COMPRA)
   if(lower_wick > body * 2 && upper_wick < body && rates[0].close > rates[0].open) {
      lastCandlePattern = "MARTILLO 🔨 (COMPRA)";
      lastCandleSignal = 1;
      lastPatternPrice = rates[0].close;
      lastPatternTime = rates[0].time;
      return;
   }
   
   // 4.2 ESTRELLA FUGAZ (señal de VENTA)
   ...
   // 4.4 ENVOLVENTE ALCISTA (señal de COMPRA)
   ...
   // 4.5 ENVOLVENTE BAJISTA (señal de VENTA)
...
//+------------------------------------------------------------------+
//| VERIFICAR SI PRECIO ESTÁ CERCA DE ZONA                           |
//+------------------------------------------------------------------+
bool IsNearFiboZone(double price, int tolerance = 10) {
   if(!fibo_valid) return false;
...
void CheckEntry() {
   ...
   bool hasGoodCandle = (lastCandleSignal == fibo_direction); // La vela apoya la dirección
   
   // Actualizar estado para HUD
   if(!inZone && !nearZone) {
      if(fibo_direction == 1) {
         botStatus = "ESPERANDO PRECIO en zona " + DoubleToString(fibo_786, _Digits) + " - " + DoubleToString(fibo_618, _Digits);
      } else {
         botStatus = "ESPERANDO PRECIO en zona " + DoubleToString(fibo_618, _Digits) + " - " + DoubleToString(fibo_786, _Digits);
      }
      return;
   }
   
   if(inZone || nearZone) {
      if(!hasConfluence && user_RequireConfluence) {
...
void CrearHUD() {
   ...
   int x = 10, y = 10, w = 540, h = 800;
   
   CrRect("bg", x, y, w, h, C'15,15,35', C'60,60,120', 2);
   
   string title = "⚡ AMETRALLADORA v3.4 | " + (isCentAccount ? "CENT" : "REAL");
