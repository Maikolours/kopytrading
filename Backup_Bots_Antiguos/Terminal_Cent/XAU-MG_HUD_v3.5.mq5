//+------------------------------------------------------------------+
//| XAU-MG_HUD_v3.5.mq5 - AMETRALLADORA                              |
...
#property description "AMETRALLADORA v3.5 - Prioridad a la Tendencia"
...
void CalculateFibonacci() {
   ...
   if(DetectImpulse(impulse_dir, start, end)) {
      ...
      // Actualizar señal esperada según tendencia
...
void UpdateExpectedSignal() {
   ...
   // Tendencia ALCISTA + Fibonacci COMPRA = alineados
   ...
   // Tendencia BAJISTA + Fibonacci VENTA = alineados
   ...
   // Tendencia ALCISTA + Fibonacci VENTA = desalineados
   else if(trend_up && fibo_direction == -1) {
      expectedSignal = "BUSCANDO NUEVO IMPULSO ALCISTA";
   }
   // Tendencia BAJISTA + Fibonacci COMPRA = desalineados
   else if(trend_down && fibo_direction == 1) {
...
void CrearHUD() {
   ...
   string title = "⚡ AMETRALLADORA v3.5 | " + (isCentAccount ? "CENT" : "REAL");
