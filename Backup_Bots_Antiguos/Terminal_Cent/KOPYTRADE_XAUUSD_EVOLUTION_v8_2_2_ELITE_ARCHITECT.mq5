//+------------------------------------------------------------------+
//|    KOPYTRADE_XAUUSD_EVOLUTION_v8_2_1 - IRON ARCHITECT           |
...
#property version   "8.22"
#property strict
#property description "ELITE ARCHITECT v8.2.2 | Premium Institutional Suite"
...
void TryConfirm(ENUM_FIBO_LEVEL lvl, ENUM_POSITION_TYPE type) {
   ...
   if(wickOk && colorOk) {
      ...
      stats[idx].entries++;
      botStatus = "L"+IntegerToString(lvl)+" ELITE ENTRY";
   }
}
...
void DrawFiboMap() {
   ObjectsDeleteAll(0, "F_LVL_");
   string dir = (f_dir == 1) ? "BUY" : "SELL";
   
   // 1. ZONA DE ORO (Golden Zone 50-61.8)
   ...
   CrFLine("61", f_61, C'255,180,0',   "61.8% - GOLD", 3);
...
void CrFLine(string n, double p, color c, string t, int w=1) {
   ...
   // Etiqueta de texto al final
   string lname = "F_LBL_"+n;
...
void CrearHUD() {
   ...
   CrRect("bg",x,y,w,h,C'15,15,30',C'100,100,200',2);
   CrLabel("ttl",x+15,y+10,"🏗️ EVO v8.2.2 ELITE ARCHITECT",C'0,255,200',11,"Impact");
   CrLabel("stL",x+15,y+45,"MODE:",C'150,150,200',8);
   CrLabel("stV",x+80,y+45,botStatus,clrWhite,9,"Arial Black");
   CrLabel("hiL",x+15,y+75,"SWING HI:",C'150,150,200',8);
   CrLabel("hi",x+100,y+75,"0.00",clrWhite,8);
   CrLabel("loL",x+15,y+95,"SWING LO:",C'150,150,200',8);
   CrLabel("lo",x+100,y+95,"0.00",clrWhite,8);
}
...
void ActualizarHUD() {
   ObjectSetString(0,PNL+"stV",OBJPROP_TEXT,"STATUS: "+botStatus);
   ObjectSetString(0,PNL+"hi",OBJPROP_TEXT,"SWING HI: "+DoubleToString(f_dir==1?f_0:f_100,_Digits));
   ObjectSetString(0,PNL+"lo",OBJPROP_TEXT,"SWING LO: "+DoubleToString(f_dir==1?f_100:f_0,_Digits));
   for(int i=1; i<=3; i++) if(stats[i].locked) botStatus = "L"+IntegerToString(i)+" LOCKED (SAFE)";
}

void CrRect(string n, int x, int y, int w, int h, color bg, color bd) {
   string name = PNL+n;
