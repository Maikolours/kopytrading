//+------------------------------------------------------------------+
//|                                   MSC_Pro_Evolution_V3_FINAL.mq5 |
//|        Versión FINAL - Cobertura + Cierre Viernes + Minutos      |
//+------------------------------------------------------------------+
...
input int      HorasMaximasOrden   = 10;    // Mantengo tus 10h para el FINAL

input group "=== CONFIGURACIÓN HORARIA PRECISA ==="
input int      HoraInicio   = 0;     
input int      MinutoInicio = 30;    // Configurado a las 00:30
input int      HoraFin      = 22;    
input int      MinutoFin    = 0;

input group "=== COBERTURA FINAL (SHIELD) ==="
...
void OnTick() {
   ...
   if(UsarCierreViernes && strTime.day_of_week == 5 && strTime.hour >= HoraCierreViernes) {
      ...
      Comment("🍹 FIN DE SEMANA: Bot en pausa hasta el lunes.");
      return;
   }

   // 2. Filtro Horario por Minutos
   ...
   // 3. Gestión de Posiciones y Cobertura
...
void EvaluarEntradaConFiltro() {
   ...
   if(precioActual > ma) {
      ...
      trade.SetExpertMagicNumber(MagicNumber);
      if(trade.Buy(LoteInicial, _Symbol, ask, 0, 0, "FINAL V3.1")) {
         ultimaOperacion = TimeCurrent();
         if(UsarCobertura) {
            double precioHedge = ask - (DistanciaPipsHedge * _Point * 10);
            trade.SetExpertMagicNumber(MagicHedge);
            trade.SellStop(LoteHedge, precioHedge, _Symbol, 0, 0, ORDER_TIME_GTC, 0, "Hedge_Final");
            trade.SetExpertMagicNumber(MagicNumber);
         }
      }
   }
...
void CerrarTodoEmergencia(string motivo) {
   for(int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      if(PositionSelectByTicket(ticket)) {
         ...
         if(mg == MagicNumber || mg == MagicHedge) trade.PositionClose(ticket);
      }
   }
   BorrarOrdenesPendientesHedge();
}
