...
void CrearHUD() {
   ...
   string accType = (accountBalanceUSD > 5000) ? " (CENT)" : " (USD)";
   string title = "⚡ STORM RIDER v1.1 | bal:" + DoubleToString(accountBalanceUSD, 2) + accType;
   ...
   int row = innerY, colLabel = 0, colValue = 135, colBtnMM = innerW - 110, colBtnM = innerW - 80, colBtnP = innerW - 45, colBtnPP = innerW - 15, btnW = 26;
   ...
   CrBtn("reset", innerX+colBtnPP-35, row-3, 50, 24, "RESET", C'80,80,120', clrWhite);
   row += 28;
   
   CrLabel("stL", innerX+colLabel, row, "ESTADO BOT:", C'150,150,200', 9);
   CrLabel("stV", innerX+colValue, row, botStatusStr, isManualPaused ? clrOrange : C'0,255,127', 9);
   CrBtn("pause", innerX+colBtnPP-55, row-3, 70, 24, isManualPaused ? "RESUMIR" : "PAUSA", isManualPaused ? C'0,150,50' : C'150,100,0', clrWhite);
   row += 28;
   ...
   for(int i=0; i<ArraySize(labels); i++) {
      ...
      // Botones de paso doble (Solo para algunos)
      if(keys[i]!="lot" && keys[i]!="mult" && keys[i]!="maxpos" && keys[i]!="rsiob" && keys[i]!="rsios") {
         CrBtn(keys[i]+"mm", innerX+colBtnMM, row-3, btnW, 24, "--", C'30,30,60', clrWhite);
         CrBtn(keys[i]+"pp", innerX+colBtnPP, row-3, btnW, 24, "++", C'30,30,60', clrWhite);
      }
      
      CrBtn(keys[i]+"m", innerX+colBtnM, row-3, btnW, 24, "-", C'40,40,70', clrWhite);
      CrBtn(keys[i]+"p", innerX+colBtnP, row-3, btnW, 24, "+", C'40,40,70', clrWhite);
      row += 28;
   }
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam) {
   if(id == CHARTEVENT_OBJECT_CLICK) {
      ...
      if(sparam == PNL_TAG + "pause") { isManualPaused = !isManualPaused; CrearHUD(); return; }
      ...
      if(sparam == PNL_TAG + "lotm") runtime_LoteManual = MathMax(0.01, runtime_LoteManual - s_tiny);
      if(sparam == PNL_TAG + "lotp") runtime_LoteManual = MathMin(2.0, runtime_LoteManual + s_tiny);
      if(sparam == PNL_TAG + "multm") runtime_HedgeMultiplier = MathMax(1.0, runtime_HedgeMultiplier - 0.1);
      if(sparam == PNL_TAG + "multp") runtime_HedgeMultiplier = MathMin(5.0, runtime_HedgeMultiplier + 0.1);

      string k[] = {"gat","meta","target","be","tsact","tsdist","dist","maxpos","rsiob","rsios"};
      for(int i=0; i<ArraySize(k); i++) {
         if(sparam == PNL_TAG + k[i] + "mm") { // -- Big
            if(k[i]=="gat") runtime_GatilloHedge_USD = MathMax(5, runtime_GatilloHedge_USD - 50);
            else if(k[i]=="meta") runtime_Net_Cycle_USD = MathMax(10, runtime_Net_Cycle_USD - 100);
            else if(k[i]=="target") runtime_Target_Individual_USD = MathMax(5, runtime_Target_Individual_USD - 50);
            else if(k[i]=="be") runtime_BE_Trigger_USD = MathMax(2, runtime_BE_Trigger_USD - 50);
            else if(k[i]=="tsact") runtime_TS_Activation_USD = MathMax(5, runtime_TS_Activation_USD - 50);
            else if(k[i]=="dist") runtime_MaxDistEMA_Points = MathMax(100, runtime_MaxDistEMA_Points - 500);
         }
         if(sparam == PNL_TAG + k[i] + "m") { // - Small
            if(k[i]=="gat") runtime_GatilloHedge_USD = MathMax(5, runtime_GatilloHedge_USD - 1);
            else if(k[i]=="meta") runtime_Net_Cycle_USD = MathMax(10, runtime_Net_Cycle_USD - 1);
            else if(k[i]=="target") runtime_Target_Individual_USD = MathMax(5, runtime_Target_Individual_USD - 1);
            else if(k[i]=="be") runtime_BE_Trigger_USD = MathMax(2, runtime_BE_Trigger_USD - 1);
            else if(k[i]=="tsact") runtime_TS_Activation_USD = MathMax(5, runtime_TS_Activation_USD - 1);
            else if(k[i]=="tsdist") runtime_TS_Distance_USD = MathMax(2, runtime_TS_Distance_USD - 1);
            else if(k[i]=="dist") runtime_MaxDistEMA_Points = MathMax(100, runtime_MaxDistEMA_Points - 100);
            else if(k[i]=="maxpos") runtime_MaxPosiciones = MathMax(1, runtime_MaxPosiciones - 1);
            else if(k[i]=="rsiob") runtime_RSI_Overbought = MathMax(60, runtime_RSI_Overbought - 1);
            else if(k[i]=="rsios") runtime_RSI_Oversold = MathMax(5, runtime_RSI_Oversold - 1);
         }
         if(sparam == PNL_TAG + k[i] + "p") { // + Small
            if(k[i]=="gat") runtime_GatilloHedge_USD = MathMin(5000, runtime_GatilloHedge_USD + 1);
            else if(k[i]=="meta") runtime_Net_Cycle_USD = MathMin(10000, runtime_Net_Cycle_USD + 1);
            else if(k[i]=="target") runtime_Target_Individual_USD = MathMin(5000, runtime_Target_Individual_USD + 1);
            else if(k[i]=="be") runtime_BE_Trigger_USD = MathMin(2000, runtime_BE_Trigger_USD + 1);
            else if(k[i]=="tsact") runtime_TS_Activation_USD = MathMin(2000, runtime_TS_Activation_USD + 1);
            else if(k[i]=="tsdist") runtime_TS_Distance_USD = MathMin(1000, runtime_TS_Distance_USD + 1);
            else if(k[i]=="dist") runtime_MaxDistEMA_Points = MathMin(5000, runtime_MaxDistEMA_Points + 100);
            else if(k[i]=="maxpos") runtime_MaxPosiciones = MathMin(10, runtime_MaxPosiciones + 1);
            else if(k[i]=="rsiob") runtime_RSI_Overbought = MathMin(95, runtime_RSI_Overbought + 1);
            else if(k[i]=="rsios") runtime_RSI_Oversold = MathMin(40, runtime_RSI_Oversold + 1);
         }
         if(sparam == PNL_TAG + k[i] + "pp") { // ++ Big
            if(k[i]=="gat") runtime_GatilloHedge_USD = MathMin(5000, runtime_GatilloHedge_USD + 50);
            else if(k[i]=="meta") runtime_Net_Cycle_USD = MathMin(10000, runtime_Net_Cycle_USD + 100);
            else if(k[i]=="target") runtime_Target_Individual_USD = MathMin(5000, runtime_Target_Individual_USD + 50);
            else if(k[i]=="be") runtime_BE_Trigger_USD = MathMin(2000, runtime_BE_Trigger_USD + 50);
            else if(k[i]=="tsact") runtime_TS_Activation_USD = MathMin(2000, runtime_TS_Activation_USD + 50);
            else if(k[i]=="dist") runtime_MaxDistEMA_Points = MathMin(5000, runtime_MaxDistEMA_Points + 500);
         }
      }
