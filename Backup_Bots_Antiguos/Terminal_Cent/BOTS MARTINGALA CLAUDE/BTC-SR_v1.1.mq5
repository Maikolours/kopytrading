...
ENUM_DIR       currentDir;
string         botName = "STORM RIDER", botStatusStr = "TRABAJANDO", commentTag = "BTC_HUD";
double         dayPnL_USD = 0;
...
void ProtectAll() {
   ...
   if(total >= 2 && netUSD >= runtime_Net_Cycle_USD) {
      CloseAllPositions();
      coolingEndTime = TimeCurrent() + 60; // 60 segundos de pausa tras éxito
   }
}
...
//+------------------------------------------------------------------+
//| Lógica de Rescate (Independiente de indicadores)                  |
//+------------------------------------------------------------------+
void CheckHedge() {
   int totalPos = GetPositionsCount();
   ...
   if(!hasHedge && (totalPos < runtime_MaxPosiciones) && mainProfitUSD <= -runtime_GatilloHedge_USD) {
      ...
      if(mainType == POSITION_TYPE_BUY)
         trade.Sell(hedgeLot, _Symbol, bid, 0, 0, commentTag + "_H");
      else
         trade.Buy(hedgeLot, _Symbol, ask, 0, 0, commentTag + "_H");
   }
}
...
void CrearHUD() {
   ...
   CrBtn("apply", innerX+5, row, innerW-10, 38, "APLICAR CONFIG", C'0,100,150', clrWhite);
   row += 46;
