...
void ProcessTelegramCommands()
{
   ...
   if(res == 200)
   {
      ...
      if(StringFind(response, "\"/status\"") != -1) { remotePaused ? SendTelegramStatus("🔴") : SendTelegramStatus("🟢"); lastUpdateID = ParseUpdateID(response); }
...
void CloseLastPosition()
{
   ...
   for(int i=0; i<PositionsTotal(); i++) {
      if(posInfo.SelectByIndex(i) && posInfo.Magic() == MagicNumber) {
         if((datetime)posInfo.IntegerProperty(POSITION_TIME) > lastTime) {
            lastTime = (datetime)posInfo.IntegerProperty(POSITION_TIME);
