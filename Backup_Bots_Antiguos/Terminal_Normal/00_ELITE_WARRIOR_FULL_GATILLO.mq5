//|             00_ELITE_WARRIOR_FULL_GATILLO                      |
//|      STRATEGY: WARRIOR PREDATOR | HUD: MASTER CHRONOS | SYNC: FULL |
//|        VERSION: 19.00 FINAL - DUAL EXECUTION MODE (MKT/LIM)      |
//+------------------------------------------------------------------+
...
ENUM_MODE      currentMode = MODE_ZEN;
...
int OnInit() {
   ...
   ApplyPreset();
   currentExec = InpExecMode;
   if(GlobalVariableCheck("WS_F_"+_Symbol)) {
      state = (ENUM_STATE)GlobalVariableGet("WS_F_"+_Symbol);
      f100 = GlobalVariableGet("WF1_F_"+_Symbol);
      f0 = GlobalVariableGet("WF0_F_"+_Symbol);
   }
   CrearPanel();
   EventSetTimer(1); 
   ChartRedraw();
...
void OnTick() {
   ...
   if(CountActive() == 0 && CountOrders() == 0) {
      if(currentExec == EXEC_MARKET) MonitorRetracement();
      else ManageLimitOrders();
   }
}
...
void DetectBOS() {
   ...
   if(ObjectFind(0,wav)<0) { ObjectCreate(0,wav,OBJ_TREND,0,iTime(_Symbol,InpTF_Mid,bull?ll:hh),f100,iTime(_Symbol,InpTF_Mid,bull?hh:ll),f0); ObjectSetInteger(0,wav,OBJPROP_WIDTH,InpWidth_Lines); ObjectSetInteger(0,wav,OBJPROP_STYLE,STYLE_DOT); }
   else { ObjectMove(0,wav,0,iTime(_Symbol,InpTF_Mid,bull?ll:hh),f100); ObjectMove(0,wav,1,iTime(_Symbol,InpTF_Mid,bull?hh:ll),f0); }
   ObjectSetInteger(0,wav,OBJPROP_COLOR,(bull?InpColor_Bull:Incolor_Bear)); ObjectSetInteger(0,wav,OBJPROP_BACK, true);
   if((bull && ask >= hi) || (!bull && bid <= lo)) { if(state != STATE_WAIT_RETRACE) { state = STATE_WAIT_RETRACE; pX_done=false; SaveState(); if(currentExec == EXEC_LIMIT) CancelOrders(); } }
}
...
void MonitorRetracement() {
   ...
   if(confirmed && p1_pierced) { if(!pX_done) { ExecuteMarket(bull, "WARRIOR_FULL"); pX_done=true; state=STATE_WAIT_BOS; SaveState(); } }
}

void ManageLimitOrders() {
   if(state != STATE_WAIT_RETRACE || CountOrders() > 0) return;
   bool bull = (dirMacro == 1);
   ExecuteLimit(bull, f61, "OTE_61.8");
   ExecuteLimit(bull, f70, "OTE_70.5");
   ExecuteLimit(bull, f78, "OTE_78.6");
}
...
void ExecuteLimit(bool bull, double price, string comment) {
   double ps = (_Digits<=2?1.0:(_Digits==3||_Digits==5?10.0*_Point:_Point));
   double lot = AccountInfoDouble(ACCOUNT_EQUITY)*InpRisk/100.0/800.0/3.0; lot = NormalizeLot(lot);
   double sl = bull ? price - InpSL_Pips*ps : price + InpSL_Pips*ps;
   double tp = bull ? price + (InpSL_Pips*InpRR_Ratio)*ps : price - (InpSL_Pips*InpRR_Ratio)*ps;
   if(bull) trade.BuyLimit(lot, price, _Symbol, sl, tp, ORDER_TIME_GTC, 0, comment); 
   else trade.SellLimit(lot, price, _Symbol, sl, tp, ORDER_TIME_GTC, 0, comment);
}
...
void SaveState() { GlobalVariableSet("WS_F_"+_Symbol, (double)state); GlobalVariableSet("WF1_F_"+_Symbol, f100); GlobalVariableSet("WF0_F_"+_Symbol, f0); }
void ManageRisk() {
   double ps = (_Digits<=2?1.0:(_Digits==3||_Digits==5?10.0*_Point:_Point));
...
void CrearPanel() {
   ...
   CrLabel("ttl", x+15, y+6, "ELITE WARRIOR FULL GATILLO", clrWhite, 9);
   ...
   if(!isMinimized) {
      ...
      CrLabel("pl_t", x+15, y+75, "🎯 ESTRATEGIA FULL GATILLO:", clrOrangeRed, 7);
      ...
      CrLabel("tr4", x+15, y+335, "BIAS MACRO (1H): " + (dirMacro==1?"ALCISTA ▲":"BAJISTA ▼"), (dirMacro==1?InpColor_Bull:Incolor_Bear), 8);
      CrLabel("tr1", x+15, y+355, "ESTRUCTURA (M15): " + (dir1H==1?"ALCISTA ▲":"BAJISTA ▼"), (dir1H==1?InpColor_Bull:Incolor_Bear), 8);
...
void UpdatePanelText() {
   ...
   ObjectSetInteger(0,HUD_PRE+"tr4",OBJPROP_COLOR,(dirMacro==1?InpColor_Bull:Incolor_Bear));
   ObjectSetString(0,HUD_PRE+"tr1",OBJPROP_TEXT,"ESTRUCTURA (M15): " + (dir1H==1?"ALCISTA ▲":"BAJISTA ▼"));
   ObjectSetInteger(0,HUD_PRE+"tr1",OBJPROP_COLOR,(dir1H==1?InpColor_Bull:Incolor_Bear));
}
...
void OnChartEvent(const int id, const long &lp, const double &dp, const string &sp) {
   ...
   if(sp == HUD_PRE+"b_mar") { currentExec = EXEC_MARKET; CancelOrders(); CrearPanel(); }
   if(sp == HUD_PRE+"b_lim") { currentExec = EXEC_LIMIT; ManageLimitOrders(); CrearPanel(); }
   if(sp == HUD_PRE+"b_c") { currentDir = DIR_COMPRAS; if(currentExec==EXEC_LIMIT){CancelOrders();ManageLimitOrders();} CrearPanel(); }
   if(sp == HUD_PRE+"b_v") { currentDir = DIR_VENTAS; if(currentExec==EXEC_LIMIT){CancelOrders();ManageLimitOrders();} CrearPanel(); }
   if(sp == HUD_PRE+"b_a") { currentDir = DIR_AMBAS; if(currentExec==EXEC_LIMIT){CancelOrders();ManageLimitOrders();} CrearPanel(); }
