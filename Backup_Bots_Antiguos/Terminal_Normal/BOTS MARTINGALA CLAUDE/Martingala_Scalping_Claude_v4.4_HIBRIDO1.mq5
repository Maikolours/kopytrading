...
//+------------------------------------------------------------------+
#property copyright "Martingala Scalping Claude v4.61 ELITE AMETRALLADORA"
...
input group "=== CONFIGURACIÓN HÍBRIDA / AMETRALLADORA ==="
input double   LoteInicial = 0.01;
input int      MaxOperaciones = 6;              // Subido para ráfaga
input double   PerdidaMaximaDiaria = 20.0;      // Margen de seguridad de $20
input double   ProfitTotalObjetivo = 5.0;       // Objetivo rápido

input bool     ModoAmetralladora = true;        // ACTIVAR RÁFAGA DE 3 TIROS
input int      NumBalas = 3;                    // Número de balas iniciales
input double   ProfitMinimo_Ganadora = 1.5;     // Cobrar rápido a partir de $1.50
input double   Perdida_ContraRapida = -50.0;    // DESACTIVADO (Solo una dirección)
input double   Ganancia_Reforzar = 2.0;         // Refuerzo cada $2
input bool     CerrarPerdedoraSiCompensa = true; // Cerrar perdedora si contraoperación compensa

input group "=== AJUSTES ORO (XAUUSD) ==="
input double   ORO_StopLoss = 50.0;             // Stop Loss alejado para seguridad
input double   ORO_BEActivacion = 0.50;
input double   ORO_TrailingStart = 3.00;        // Trailing desde $3.00
input double   ORO_MultiplicadorLote = 1.5;     // Más agresivo (antes 1.8)

input group "=== FILTRO HORARIO (APERTURAS) ==="
input bool     EvitarAperturas = true;          // Bloquear trading en aperturas
input int      ServerHoraNY = 15;               // Hora server apertura NY (15:30)
input int      ServerHoraLon = 10;              // Hora server apertura Lon (10:00)

input group "=== CONTROL ==="
input bool     UsarFiltroVelas = true;          // Evitar Dojis y mechas largas
input double   MaxSpread = 5.0;                 // Máximo spread permitido (en $)
...
int OnInit()
{
    ...
    Print("═══════════════════════════════════════");
    Print("  MSC v4.61 ELITE AMETRALLADORA");
...
void OnTick()
{
    ...
    // Filtro horario automático
    if(EvitarAperturas && !EsHoraSegura())
    {
        ObjectSetString(0, "LabelEstado", OBJPROP_TEXT, "⏸️ FILTRO APERTURA");
        ObjectSetInteger(0, "LabelEstado", OBJPROP_COLOR, clrOrange);
        return;
    }
...
void GestionarOperacionesHibridas()
{
    ...
    if(peorProfitCompra <= PerdidaParaContra && numVentas == 0)
    {
        ...
        AbrirPosicion(ORDER_TYPE_SELL, bid);
    ...
    if(peorProfitVenta <= PerdidaParaContra && numCompras == 0)
    {
        ...
        AbrirPosicion(ORDER_TYPE_BUY, ask);
        ultimoRefuerzo = ahora;
        return;
    }
...
{
    if(EvitarAperturas && !EsHoraSegura()) return;
    
    double close[];
    ArraySetAsSeries(close, true);
    if(CopyClose(_Symbol, _Period, 0, 5, close) <= 0) return;
    ...
    {
        if(UsarFiltroVelas && !FiltroVelaSegura()) return;
        Print("🔵 RÁFAGA #1 COMPRA | ", NumBalas, " balas lanzadas!");
        
        int disparos = ModoAmetralladora ? NumBalas : 1;
        for(int i = 0; i < disparos; i++) {
            AbrirPosicion(ORDER_TYPE_BUY, ask);
            Sleep(500); // Pequeña pausa para no saturar al broker
        }
    }
    else if(momentumPips <= -MomentumMinimoPips)
    {
        if(UsarFiltroVelas && !FiltroVelaSegura()) return;
        Print("🔴 RÁFAGA #1 VENTA | ", NumBalas, " balas lanzadas!");
...
//+------------------------------------------------------------------+
// NUEVO: Filtro de velas seguras (Evita Dojis y Mechas largas)
//+------------------------------------------------------------------+
bool FiltroVelaSegura()
{
    ...
    double rangoTotal = high - low;
    
    if(rangoTotal <= 0) return false;
    
    double ratioCuerpo = cuerpo / rangoTotal;
    
    // Si el cuerpo es menos del 20% de la vela, es una Doji o tiene mucha mecha
    if(ratioCuerpo < 0.20)
    {
        Print("🕯️ FILTRO VELA: Vela con mucha mecha o Doji detectada (Ratio: ", DoubleToString(ratioCuerpo, 2), "). Esperando...");
        return false;
    }
...
//+------------------------------------------------------------------+
// NUEVO: Filtro horario de seguridad
//+------------------------------------------------------------------+
bool EsHoraSegura()
{
    MqlDateTime dt;
    ...
    // Filtro Londres (10 min antes y 10 min después de las 10:00 server)
    if(dt.hour == ServerHoraLon - 1 && dt.min >= 50) return false;
    if(dt.hour == ServerHoraLon && dt.min <= 10) return false;
    
    // Filtro Nueva York (10 min antes y 10 min después de las 15:30 server)
    if(dt.hour == ServerHoraNY && dt.min >= 20 && dt.min <= 40) return false;
    
    // Viernes tarde (Cierre de mercado) - No operar después de las 20:00 server
...
//+------------------------------------------------------------------+
void AbrirPosicion(ENUM_ORDER_TYPE tipo, double precio)
{
    // Filtro de Spread
    double spread = (SymbolInfoDouble(_Symbol, SYMBOL_ASK) - SymbolInfoDouble(_Symbol, SYMBOL_BID)) / (SymbolInfoDouble(_Symbol, SYMBOL_POINT) * 10);
    if(spread > MaxSpread)
    {
        Print("⚠️ SPREAD ALTO: ", DoubleToString(spread, 1), " (Máx: ", MaxSpread, "). Abortando...");
        return;
    }
    
    double lote = LoteInicial;
    if(totalOperaciones > 0) lote = CalcularSiguienteLote();
