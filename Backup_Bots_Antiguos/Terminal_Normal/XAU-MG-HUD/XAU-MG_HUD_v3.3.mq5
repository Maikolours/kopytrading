//+------------------------------------------------------------------+
//| XAU-MG_HUD_v3.3.mq5 - AMETRALLADORA                              |
//| Versión Profesional Completa - Fibonacci OTE + Confluencia       |
//| + Velas de Rechazo + Extensiones + SMA200                        |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, Kopytrading Corp."
#property link      "https://www.kopytrading.com"
#property version   "3.30"
#property description "AMETRALLADORA v3.3 - Bot Profesional Completo"
#property description "Fibonacci OTE | Confluencia Múltiple | Velas de Rechazo"
#property description "Extensiones Fibonacci | SMA200 | Multi-TF"
...
double   user_Fibo_272 = 0.272;   // Extensión 27.2%
double   user_Fibo_618_ext = 0.618; // Extensión 61.8%
...
CSymbolInfo    symInfo;
int            activeMagic, h_ema20, h_ema50, h_rsi, h_atr, h_ema_h1, h_sma200;
...
int            rejectionSignal = 0;     // 1=compra, -1=venta, 0=nada
...
//+------------------------------------------------------------------+
//| 1. TENDENCIA H1 (EMA 50 + SMA200)                                |
//+------------------------------------------------------------------+
string GetTrendH1() {
   double emaH1[1], sma200[1];
   if(CopyBuffer(h_ema_h1, 0, 0, 1, emaH1) <= 0) return "DESCONOCIDA";
   if(CopyBuffer(h_sma200, 0, 0, 1, sma200) <= 0) return "DESCONOCIDA";
   
   double price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   bool ema_up = (price > emaH1[0]);
   bool sma_up = (price > sma200[0]);
   
   if(ema_up && sma_up) return "ALCISTA ▲ (EMA+SMA)";
   if(!ema_up && !sma_up) return "BAJISTA ▼ (EMA+SMA)";
   if(ema_up && !sma_up) return "ALCISTA DÉBIL (solo EMA)";
   return "BAJISTA DÉBIL (solo SMA)";
}

//+------------------------------------------------------------------+
//| 2. DETECTAR IMPULSO EN M15 (con mechas)                          |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| 3. CALCULAR FIBONACCI (OTE: 61.8% - 78.6%)                       |
//+------------------------------------------------------------------+
void CalculateFibonacci() {
   ...
   if(DetectImpulse(impulse_dir, start, end)) {
      ...
      fibo_valid = true;
      botStatus = "FIBO ACTIVO";
   }
}

//+------------------------------------------------------------------+
//| 4. VERIFICAR CONFLUENCIA MÚLTIPLE                                |
//+------------------------------------------------------------------+
void CheckConfluence(double price) {
   ...
   // EMA20
   if(MathAbs(price - ema20[0]) < 8 * _Point) {
      confluenceCount++;
      confluenceSources += "EMA20 ";
   }
   
   // EMA50
   if(MathAbs(price - ema50[0]) < 8 * _Point) {
      confluenceCount++;
      confluenceSources += "EMA50 ";
   }
   
   // Números redondos (Oro: 4500, 4550, 4600, etc.)
   double round_level = MathRound(price / 50) * 50;
   if(MathAbs(price - round_level) < 5 * _Point) {
      confluenceCount++;
      confluenceSources += "REDONDO ";
   }
   
   // Soportes/resistencias H1
   double highs[], lows[];
   if(CopyHigh(_Symbol, PERIOD_H1, 0, 100, highs) >= 100 &&
      CopyLow(_Symbol, PERIOD_H1, 0, 100, lows) >= 100) {
      for(int i = 0; i < 100; i++) {
         if(MathAbs(price - highs[i]) < 8 * _Point) {
            confluenceCount++;
            confluenceSources += "RESISTENCIA ";
            break;
         }
         if(MathAbs(price - lows[i]) < 8 * _Point) {
            confluenceCount++;
            confluenceSources += "SOPORTE ";
            break;
         }
      }
   }
...
//+------------------------------------------------------------------+
//| 5. DETECTAR VELA DE RECHAZO (Martillo, Estrella, Envolvente)     |
//+------------------------------------------------------------------+
int DetectRejectionCandle() {
   MqlRates rates[2];
   ...
   // Martillo (COMPRA)
   if(lower_wick > body * 2 && rates[0].close > rates[0].open) {
      currentCandlePattern = "MARTILLO";
      return 1;
   }
   
   // Estrella fugaz (VENTA)
   if(upper_wick > body * 2 && rates[0].close < rates[0].open) {
      currentCandlePattern = "ESTRELLA FUGAZ";
      return -1;
   }
   
   // Envolvente alcista (COMPRA)
   ...
   if(prev_bearish && curr_bullish && engulfs_up) {
      currentCandlePattern = "ENVOLVENTE ALCISTA";
      return 1;
   }
   
   // Envolvente bajista (VENTA)
   ...
   if(prev_bullish && curr_bearish && engulfs_down) {
      currentCandlePattern = "ENVOLVENTE BAJISTA";
      return -1;
   }
   
   currentCandlePattern = "SIN SEÑAL";
   return 0;
}

//+------------------------------------------------------------------+
//| VERIFICAR SI PRECIO ESTÁ EN ZONA FIBO                            |
//+------------------------------------------------------------------+
...
void UpdateHUDInfo() {
   ...
   CheckConfluence(priceNow);
   rejectionSignal = DetectRejectionCandle();
   
   bool inZone = IsInFiboZone(priceNow);
   bool hasConfluence = (confluenceCount >= 1);
   bool hasRejection = (rejectionSignal == fibo_direction);
   
   if(!fibo_valid) {
      botStatus = "DETECTANDO IMPULSO...";
   } else if(!inZone) {
      if(fibo_direction == 1) {
         botStatus = "ESPERANDO RETROCESO a " + DoubleToString(fibo_786, _Digits) + " - " + DoubleToString(fibo_618, _Digits);
      } else {
         botStatus = "ESPERANDO RETROCESO a " + DoubleToString(fibo_618, _Digits) + " - " + DoubleToString(fibo_786, _Digits);
      }
   } else if(inZone && !hasConfluence) {
      botStatus = "EN ZONA FIBO, SIN CONFLUENCIA";
   } else if(inZone && hasConfluence && !hasRejection) {
      botStatus = "ZONA FIBO + CONFLUENCIA, ESPERANDO VELA DE RECHAZO";
   } else if(inZone && hasConfluence && hasRejection) {
      botStatus = "✅ TODAS CONDICIONES LISTAS - LISTO PARA ENTRAR";
   }
}
...
//+------------------------------------------------------------------+
//| ENTRADA CON TODAS LAS CONFIRMACIONES                              |
//+------------------------------------------------------------------+
void CheckEntry() {
   if(GetPositionsCount() >= runtime_MaxPosiciones) return;
   ...
   if(!inZone || !hasConfluence || !hasRejection) return;
   ...
   if(fibo_direction == 1) {
      if(rsi[0] < 50 || rsi[0] > runtime_RSI_Overbought) return;
   } else {
      if(rsi[0] > 50 || rsi[0] < runtime_RSI_Oversold) return;
   }
   ...
   if(fibo_direction == 1 && emaF[0] <= emaS[0]) return;
   if(fibo_direction == -1 && emaF[0] >= emaS[0]) return;
   ...
   if(fibo_direction == 1) {
      if(trade.Buy(runtime_LoteManual, _Symbol, ask, fibo_786, tp1, "FIBO_ENTRY")) {
         lastTradeClose = 0;
         botStatus = "✅ COMPRA EJECUTADA en zona Fibonacci";
      }
   } else {
      if(trade.Sell(runtime_LoteManual, _Symbol, bid, fibo_618, tp1, "FIBO_ENTRY")) {
         lastTradeClose = 0;
         botStatus = "✅ VENTA EJECUTADA en zona Fibonacci";
      }
   }
}

//+------------------------------------------------------------------+
//| HUD COMPLETO PROFESIONAL                                         |
//+------------------------------------------------------------------+
void CrearHUD() {
   ObjectsDeleteAll(0, PNL_TAG);
   int x = 10, y = 10, w = 520, h = 780;
   
   CrRect("bg", x, y, w, h, C'15,15,35', C'60,60,120', 2);
   
   string title = "⚡ AMETRALLADORA v3.3 | " + (isCentAccount ? "CENT" : "REAL");
   ...
   int row = innerY, colLabel = 0, colValue = 170, colEdit = innerW - 100;
   ...
   // Tendencia
   CrLabel("trL", innerX+colLabel, row, "🧭 TENDENCIA H1:", C'150,150,200', 10);
   CrLabel("trV", innerX+colValue, row, GetTrendH1(), clrWhite, 9);
   row += 24;
   ...
   CrLabel("cndV", innerX+colValue, row, currentCandlePattern, (rejectionSignal != 0) ? C'100,255,100' : C'255,200,100', 9);
   row += 28;
...
void ActualizarHUD() {
   ...
   ObjectSetString(0, PNL_TAG + "trV", OBJPROP_TEXT, GetTrendH1());
   ...
   ObjectSetString(0, PNL_TAG + "cndV", OBJPROP_TEXT, currentCandlePattern);
}
...
int OnInit() {
   ...
   h_sma200 = iMA(_Symbol, PERIOD_H1, 200, 0, MODE_SMA, PRICE_CLOSE);
