//+------------------------------------------------------------------+
//| XAU-MG_HUD_v3.7.mq5 - AMETRALLADORA                              |
//| Versión Equilibrada - Fibonacci H1 + Entradas más frecuentes     |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, Kopytrading Corp."
#property link      "https://www.kopytrading.com"
#property version   "3.70"
#property description "AMETRALLADORA v3.7 - Fibonacci H1 | Entradas equilibradas"
#property description "Fibonacci basado en H1 (50 velas) | Confluencia recomendada"
...
//--- PARÁMETROS DE ESTRATEGIA (AJUSTADOS PARA MÁS ENTRADAS)
...
double   user_MaxDistEMA_Points  = 800;      // Aumentado de 500 a 800
...
//+------------------------------------------------------------------+
//| 2. FIBONACCI BASADO EN H1 (50 velas = 50 horas ≈ 2 días)        |
//+------------------------------------------------------------------+
void CalculateFibonacciH1() {
   double highs[], lows[];
   int bars = 50;  // 50 velas H1 = 50 horas ≈ 2 días
   
   if(CopyHigh(_Symbol, PERIOD_H1, 0, bars, highs) < bars) return;
   if(CopyLow(_Symbol, PERIOD_H1, 0, bars, lows) < bars) return;
   
   // Encontrar máximo y mínimo en las últimas 50 velas H1
   int max_idx = ArrayMaximum(highs);
   int min_idx = ArrayMinimum(lows);
   double max_h1 = highs[max_idx];
   double min_h1 = lows[min_idx];
   
   // Determinar dirección basada en la tendencia actual
   double emaF[1], emaS[1];
   ...
   if(trend_up) {
      // Tendencia alcista: Fibonacci desde mínimo hasta máximo
      fibo_direction = 1;
      fibo_0 = min_h1;
      fibo_100 = max_h1;
   } else {
      // Tendencia bajista: Fibonacci desde máximo hasta mínimo
      fibo_direction = -1;
      fibo_0 = max_h1;
      fibo_100 = min_h1;
   }
   
   double range = MathAbs(fibo_100 - fibo_0);
   
   if(fibo_direction == 1) {
      fibo_618 = fibo_100 - range * user_Fibo_618;
      fibo_786 = fibo_100 - range * user_Fibo_786;
      tp1 = fibo_100 + range * user_Fibo_272;
      tp2 = fibo_100 + range * user_Fibo_618_ext;
   ...
   Print("📊 Fibonacci H1 actualizado | Max: ", fibo_100, " | Min: ", fibo_0, " | Dir: ", (fibo_direction == 1 ? "COMPRA" : "VENTA"));
}

//+------------------------------------------------------------------+
//| 3. ACTUALIZAR SEÑAL ESPERADA                                     |
//+------------------------------------------------------------------+
void UpdateExpectedSignal() {
   if(!fibo_valid) {
      expectedSignal = "CALCULANDO FIBONACCI H1...";
      return;
   }
...
//+------------------------------------------------------------------+
//| 4. DIBUJAR FIBONACCI EN EL GRÁFICO                               |
//+------------------------------------------------------------------+
void DrawFibonacciLevels() {
   ...
   // Texto informativo
   string info = "FIBO H1 | " + (fibo_direction == 1 ? "COMPRA" : "VENTA") + " | Zona: " + DoubleToString(fibo_786, _Digits) + " - " + DoubleToString(fibo_618, _Digits);
   ObjectCreate(0, "FIBO_INFO", OBJ_LABEL, 0, 0, 0);
   ObjectSetInteger(0, "FIBO_INFO", OBJPROP_XDISTANCE, 10);
   ObjectSetInteger(0, "FIBO_INFO", OBJPROP_YDISTANCE, 80);
   ObjectSetString(0, "FIBO_INFO", OBJPROP_TEXT, info);
   ObjectSetInteger(0, "FIBO_INFO", OBJPROP_COLOR, C'255,200,0');
   ObjectSetInteger(0, "FIBO_INFO", OBJPROP_FONTSIZE, 9);
}

//+------------------------------------------------------------------+
//| 5. ANÁLISIS PROFESIONAL DE VELAS (M5)                            |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| 6. VERIFICAR CONFLUENCIA (recomendada, no obligatoria)           |
//+------------------------------------------------------------------+
void CheckConfluence(double price) {
   ...
   if(MathAbs(price - ema20[0]) < 20 * _Point) {
      confluenceCount++;
      confluenceSources += "EMA20 ";
   }
   
   if(MathAbs(price - ema50[0]) < 20 * _Point) {
      confluenceCount++;
      confluenceSources += "EMA50 ";
   }
   
   double round_level = MathRound(price / 50) * 50;
   if(MathAbs(price - round_level) < 15 * _Point) {
      confluenceCount++;
...
//+------------------------------------------------------------------+
//| 7. VERIFICAR SI PRECIO ESTÁ EN ZONA FIBO                         |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| 8. VERIFICAR SI PRECIO ESTÁ CERCA DE ZONA                        |
//+------------------------------------------------------------------+
bool IsNearFiboZone(double price, int tolerance = 20) {
   if(!fibo_valid) return false;
...
//+------------------------------------------------------------------+
//| 9. ENTRADA CON CRITERIOS EQUILIBRADOS                             |
//+------------------------------------------------------------------+
void CheckEntry() {
   if(GetPositionsCount() >= runtime_MaxPosiciones) return;
   ...
   if((trend_up && fibo_direction == -1) || (trend_down && fibo_direction == 1)) {
      botStatus = "TENDENCIA Y FIBO DESALINEADOS - ESPERANDO";
      return;
   }
   
   // Verificar si el precio está en zona o cerca
   bool inZone = IsInFiboZone(priceNow);
   bool nearZone = IsNearFiboZone(priceNow, 25);  // Tolerancia aumentada
   ...
   if(inZone || nearZone) {
      Print("🔍 Zona alcanzada | Precio: ", priceNow, " | Zona: ", fibo_786, " - ", fibo_618, " | Confluencia: ", confluenceCount, " | Vela: ", lastCandlePattern);
   }
   
   // Confluencia: recomendada pero no obligatoria (comentamos la condición)
   // if((inZone || nearZone) && user_RequireConfluence && !hasConfluence) {
   //    botStatus = "EN ZONA FIBO, SIN CONFLUENCIA (esperando)";
   //    return;
   // }
   
   // Vela de confirmación: recomendada pero puede entrar con vela neutra si está muy cerca
   if((inZone || nearZone) && !hasGoodCandle && !inZone) {
      botStatus = "CERCA DE ZONA FIBO, VELA: " + lastCandlePattern + " (esperando mejor confirmación)";
      return;
   }
   ...
   if(fibo_direction == 1) {
      if(rsi[0] < 45 || rsi[0] > runtime_RSI_Overbought) {  // RSI mínimo bajado a 45
         botStatus = "RSI FUERA DE RANGO (" + DoubleToString(rsi[0], 1) + ")";
         return;
      }
   } else {
      if(rsi[0] > 55 || rsi[0] < runtime_RSI_Oversold) {   // RSI máximo subido a 55
         botStatus = "RSI FUERA DE RANGO (" + DoubleToString(rsi[0], 1) + ")";
         return;
      }
   }
...
void CrearHUD() {
   ...
   string title = "⚡ AMETRALLADORA v3.7 | " + (isCentAccount ? "CENT" : "REAL");
   ...
   CrLabel("fibL", innerX+colLabel, row, "📊 FIBONACCI H1:", C'150,150,200', 10);
...
void OnTick() {
   ...
   // Actualizar Fibonacci H1 cada hora
   if(TimeCurrent() % 3600 == 0) {
      CalculateFibonacciH1();
   }
