//|                  00_ELITE_APEX_v16.30_WARRIOR                    |
//|      STRATEGY: WARRIOR PREDATOR | HUD: ELITE COMMAND | SYNC: FULL |
//+------------------------------------------------------------------+
...
void DetectBOS() {
   ...
   ObjectSetInteger(0,wav,OBJPROP_COLOR,(bull?clrCyan:clrOrangeRed));
   ObjectSetInteger(0,wav,OBJPROP_BACK, true);

   if((bull && ask >= hi) || (!bull && bid <= lo)) {
      if(state != STATE_WAIT_RETRACE) { 
         state = STATE_WAIT_RETRACE; 
         pX_done=p0_done=p1_done=p2_done=p3_done=false;
         pX_pierced=p0_pierced=p1_pierced=p2_pierced=p3_pierced=false;
         SaveState(); 
      }
   }
}
...
void DrawFibo(double p100, double p0, bool bull) {
   if(ObjectFind(0, FIBO_NAME)<0) {
      ...
      ObjectSetDouble(0, FIBO_NAME, OBJPROP_LEVELVALUE, 0, 0.0);    ObjectSetString(0, FIBO_NAME, OBJPROP_LEVELTEXT, 0, "0.0 TP");
      ...
      ObjectSetDouble(0, FIBO_NAME, OBJPROP_LEVELVALUE, 7, 1.0);    ObjectSetString(0, FIBO_NAME, OBJPROP_LEVELTEXT, 7, "100.0 SL");
   }
   ...
   ObjectSetInteger(0, FIBO_NAME, OBJPROP_COLOR, clrBlack); 
   ...
   DrawZone("ZX", f23, bull?f23-120*_Point:f23+120*_Point, bull);
   DrawZone("Z0", f38, bull?f38-120*_Point:f38+120*_Point, bull);
   DrawZone("Z1", f61, bull?f61-120*_Point:f61+120*_Point, bull);
   DrawZone("Z2", f70, bull?f70-120*_Point:f70+120*_Point, bull);
   DrawZone("Z3", f78, bull?f78-120*_Point:f78+120*_Point, bull);
   
   DrawTacticalLine("LX", f23, bull, "IMP x");
   DrawTacticalLine("L0", f38, bull, "IMP 0");
   DrawTacticalLine("L1", f61, bull, "OTE 1");
   DrawTacticalLine("L2", f70, bull, "OTE 2");
   DrawTacticalLine("L3", f78, bull, "OTE 3");
}

void DrawZone(string n, double p1, double p2, bool bull) {
   string nm = GRAF_PRE+"Z_"+n;
   color cl = (bull?C'30,25,0':C'30,10,0');
...
void DrawTacticalLine(string n, double entry, bool bull, string ote) {
   ...
   double offset = 10 * ps; 
   double vis_entry = bull ? entry + offset : entry - offset; 
   string label = (bull ? "BUY LIMIT " : "SELL LIMIT ") + ote;
   string enm = GRAF_PRE+"E_"+n;
...
void ExecuteMarket(bool bull, string comment) {
   ...
   double sl = bull ? f100 - InpSL_Pips*ps : f100 + InpSL_Pips*ps;
   double tp = bull ? f0 + 200*ps : f0 - 200*ps; 
...
void ManageRisk() {
   ...
   for(int i=PositionsTotal()-1; i>=0; i--) if(posInfo.SelectByIndex(i) && posInfo.Magic()==InpMagic) {
      double op=posInfo.PriceOpen(), cur=posInfo.PriceCurrent(), sl=posInfo.StopLoss();
      double pnl = (posInfo.PositionType()==POSITION_TYPE_BUY) ? (cur-op)/ps : (op-cur)/ps;
...
void CrearPanel() {
   ...
   int x=15, y=30, w=355, h=isMinimized ? 45 : 625;
   CrRect("bg", x, y, w, h, C'18,10,10', C'220,40,40', 2);
   CrRect("hdr", x+2, y+2, w-4, 40, C'45,15,15', C'70,20,20');
   CrLabel("ttl", x+15, y+8, "ELITE APEX v16.30 WARRIOR", clrWhite, 10);
   CrBtn("b_min", x+w-35, y+8, 25, 25, isMinimized ? "▢" : "_", C'100,20,20', clrWhite);
   if(!isMinimized) {
      CrRect("pl_bg", x+10, y+75, w-20, 105, C'30,10,10', C'120,30,30');
      CrLabel("pl_t", x+20, y+80, "🎯 ESTRATEGIA WARRIOR PREDATOR:", clrOrangeRed, 7);
      string n1 = StringSubstr(botNarrative,0,50);
      CrLabel("planA", x+20, y+100, n1, clrWhite, 8); 
      CrLabel("vis", x+20, y+120, "VISIÓN: "+(string)curHours+"h | PROTECCIÓN: "+(string)curBE+"pts", clrGray, 7);
      CrBtn("b_day", x+10, y+190, 165, 30, "MODO DIARIO", currentPreset==PRESET_DIARIO?clrCyan:C'50,15,15', clrWhite);
      CrBtn("b_we", x+180, y+190, 165, 30, "MODO FINDE", currentPreset==PRESET_FINDE?clrOrangeRed:C'50,15,15', clrWhite);
      CrRect("ote_bg", x+10, y+230, w-20, 105, C'20,20,20', C'80,80,20');
      ...
      CrLabel("tr4", x+15, y+370, "BIAS MACRO (1H): " + (dirMacro==1?"ALCISTA ▲":"BAJISTA ▼"), (dirMacro==1?clrCyan:clrOrangeRed), 9);
      CrLabel("tr1", x+15, y+395, "ESTRUCTURA (M15): " + (dir1H==1?"ALCISTA ▲":"BAJISTA ▼"), (dir1H==1?clrCyan:clrOrangeRed), 9);
      CrBtn("b_zen", x+10, y+430, 165, 30, "MODO ZEN", currentMode==MODE_ZEN?clrCrimson:C'50,15,15', clrWhite);
      CrBtn("b_har", x+180, y+430, 165, 30, "COSECHA", currentMode==MODE_COSECHA?clrOrangeRed:C'50,15,15', clrWhite);
      CrBtn("b_mar", x+10, y+470, 165, 30, "MERCADO (Gatillo)", currentExec==EXEC_MARKET?clrCrimson:C'50,15,15', clrWhite);
      CrBtn("b_lim", x+180, y+470, 165, 30, "AUTO (Zonas)", currentExec==EXEC_LIMIT?clrCrimson:C'50,15,15', clrWhite);
      CrBtn("b_c", x+10, y+510, 108, 30, "COMPRA", currentDir==DIR_COMPRAS?clrCrimson:C'50,15,15', clrWhite);
      CrBtn("b_a", x+123, y+510, 108, 30, "AMBAS", currentDir==DIR_AMBAS?clrCrimson:C'50,15,15', clrWhite);
      CrBtn("b_v", x+236, y+510, 108, 30, "VENTA", currentDir==DIR_VENTAS?clrCrimson:C'50,15,15', clrWhite);
      CrBtn("b_cl", x+10, y+555, 335, 60, "CERRAR TODO (PANIC)", C'255,20,20', clrWhite);
   }
}
...
void OnChartEvent(const int id, const long &lp, const double &dp, const string &sp) {
   ...
   if(sp == HUD_PRE+"b_cl") { for(int i=PositionsTotal()-1; i>=0; i--) if(posInfo.SelectByIndex(i) && posInfo.Magic()==InpMagic) trade.PositionClose(posInfo.Ticket()); CancelOrders(); }
