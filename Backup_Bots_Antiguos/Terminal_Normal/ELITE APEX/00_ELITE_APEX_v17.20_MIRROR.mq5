...
void MonitorRetracement() {
   ...
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK), bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   bool bull = (dirMacro == 1); // El gatillo depende del Bias
   double currentPrice = bull ? ask : bid;
   // Lógica de Giro: Si es bajista, esperamos que el precio SUBA a las zonas superiores
   ...
   if(confirmed && p1_pierced) { if(!pX_done) { ExecuteMarket(bull, "OTE_MIRROR"); pX_done=true; state=STATE_WAIT_BOS; SaveState(); } }
}
...
void CrearPanel() {
   ...
   CrLabel("ttl", x+15, y+8, "ELITE APEX v17.20 MIRROR", clrWhite, 10);
   ...
   if(!isMinimized) {
      ...
      CrLabel("planA", x+25, y+100, botNarrative, clrWhite, 7); 
