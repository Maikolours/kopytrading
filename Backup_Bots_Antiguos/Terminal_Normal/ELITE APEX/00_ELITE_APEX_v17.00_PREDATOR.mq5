...
input int             InpLkb_WE        = 6;   // Reducido para más actividad en finde
input int             InpBE_WE         = 200; // Más sensible para BTC finde
...
void DetectBOS() {
   ...
   // --- CAMBIO A SHIFT 0 PARA DETECCIÓN EN TIEMPO REAL ---
...
void MonitorRetracement() {
   ...
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK), bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   bool bull = (f0 > f100);
   double currentPrice = bull ? ask : bid;
   // Lógica de Giro: Solo si el precio ha entrado en la zona de profundidad (78.6 o superior) y vuelve a girar
   ...
   if(confirmed && p1_pierced) { if(!pX_done) { ExecuteMarket(bull, "OTE_SNIPER"); pX_done=true; state=STATE_WAIT_BOS; SaveState(); } }
}
...
void CrearPanel() {
   ...
   CrLabel("ttl", x+15, y+8, "ELITE APEX v17.00 PREDATOR", clrWhite, 10);
...
void UpdatePanelText() {
   ...
   long sec = iTime(_Symbol, InpTF_Exec, 0) + PeriodSeconds(InpTF_Exec) - TimeCurrent(); if(sec < 0) sec = 0;
