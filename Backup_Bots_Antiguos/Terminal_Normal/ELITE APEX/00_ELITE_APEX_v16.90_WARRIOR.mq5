...
bool           pX_done=false, p0_done=false, p1_done=false, p1_pierced=false, p2_pierced=false, p3_pierced=false;
...
void UpdateTrends() {
   double eM[1], e1[1];
   // Usamos el Buffer 0 de la EMA para obtener el valor ACTUAL en tiempo real
   if(CopyBuffer(hEmaMacro, 0, 0, 1, eM) <= 0 || CopyBuffer(hEma1H, 0, 0, 1, e1) <= 0) return;
   double price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   // Comparación DIRECTA precio vs EMA para respuesta instantánea
...
void DetectBOS() {
   ...
   if((bull && ask >= hi) || (!bull && bid <= lo)) { if(state != STATE_WAIT_RETRACE) { state = STATE_WAIT_RETRACE; pX_done=p0_done=p1_done=false; SaveState(); } }
}
...
void MonitorRetracement() {
   ...
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK), bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   bool bull = (f0 > f100);
   if(bull) { if(ask <= f23) p1_pierced=true; if(ask <= f38) p2_pierced=true; if(ask <= f78) p3_pierced=true; }
   else { if(bid >= f23) p1_pierced=true; if(bid >= f38) p2_pierced=true; if(bid >= f78) p3_pierced=true; }
   double c0=iClose(_Symbol,InpTF_Exec,0), o0=iOpen(_Symbol,InpTF_Exec,0); bool confirmed = bull ? (c0>o0) : (c0<o0);
   if(confirmed) {
      if(!pX_done && confirmed) { ExecuteMarket(bull, "OTE_GIRO"); pX_done=true; state=STATE_WAIT_BOS; SaveState(); }
   }
}
...
void CrearPanel() {
   ...
   CrLabel("ttl", x+15, y+8, "ELITE APEX v16.90 WARRIOR", clrWhite, 10);
   ...
   if(!isMinimized) {
      ...
      string b1 = "✔ BIAS ALCISTA: Esperando giro Sniper en zonas."; 
      if(dirMacro == -1) b1 = "✔ BIAS BAJISTA: Esperando giro Sniper en zonas.";
...
void UpdatePanelText() {
   ...
   UpdateTrends(); // Actualizamos tendencias para que el HUD sea reactivo instantáneo
}
