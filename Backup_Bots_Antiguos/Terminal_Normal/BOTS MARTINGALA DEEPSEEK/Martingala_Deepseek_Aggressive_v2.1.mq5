//+------------------------------------------------------------------+
//|              Martingala_Deepseek_Aggressive_v2.1.mq5            |
//|               VERSIÓN AGRESIVA - COMPLETAMENTE CONFIGURABLE     |
//+------------------------------------------------------------------+
#property copyright "Martingala Deepseek Aggressive v2.1"
...
input double   StopLossPorOperacion = -8.0;      // Cierre perdedoras a -$X
...
input double   TakeProfitPips = 150.0;           // TP en pips (15 pips reales)
input bool     UsarBreakEven = true;
input double   BEActivacionProfit = 2.0;         // $ para activar BE
input double   BEPips = 20.0;                    // Pips para mover SL a BE (2 pips)
input bool     UsarTrailingStop = true;
input double   TrailingStartProfit = 5.0;        // $ para iniciar trailing
input double   TrailingStepPips = 50.0;          // Pips de paso trailing (5 pips)

input group "=== CONFIGURACIÓN OPERATIVA ==="
input double   MultiplicadorLoteMaximo = 2.0;    // Multiplicador máximo
input int      CooldownSegundos = 30;            // Espera entre operaciones
...
input bool     EvitarHorasNoticias = false;
input int      HoraInicioTrading = 1;
input int      HoraFinTrading = 22;
input double   MomentumMinimoPips = 1.0;         // Momento mínimo entrada
...
int OnInit()
{
    ...
    Print("═══════════════════════════════════════════════════");
    Print("        DEEPSEEK AGGRESSIVE v2.1");
    Print("         VERSIÓN CON GESTIÓN DE GANANCIAS MEJORADA");
    Print("═══════════════════════════════════════════════════");
    Print("Par: ", NombrePar);
    ...
    Print("Take Profit Diario: $", TakeProfitDiario);
    ...
    Print("═══════════════════════════════════════════════════");
    Print("NUEVAS REGLAS DE CIERRE:");
    Print(" - Take Profit: ", TakeProfitPips/10, " pips reales");
    Print(" - Trailing Start: $", TrailingStartProfit);
    Print(" - Break Even: $", BEActivacionProfit);
    Print(" - Cierre mínimo: $", ProfitMinimoCierre);
    Print("═══════════════════════════════════════════════════");
...
void OnTick()
{
    ...
    // Nueva función: Cierre de ganancias mejorado
...
//+------------------------------------------------------------------+
//| CIERRE DE GANANCIAS MEJORADO - GANANCIAS MÁS SIGNIFICATIVAS     |
//+------------------------------------------------------------------+
void CerrarGanadorasMejorado()
{
    ...
    for(int i = ArraySize(posiciones) - 1; i >= 0; i--)
    {
        ...
        // REGLA 1: Cerrar si ganancia > $8 y tiempo > 1 minuto
        if(segundos >= 60 && posiciones[i].profit >= 8.0)
        {
            ...
            if(trade.PositionClose(posiciones[i].ticket))
            {
                ...
                Print("💰 GANANCIA CERRADA: $", DoubleToString(profitCierre, 2), 
                      " en ", segundos, "s (Regla 1)");
            }
        }
        // REGLA 2: Cerrar si gran ganancia > $15 inmediatamente
        else if(posiciones[i].profit >= 15.0)
        {
            double profitCierre = posiciones[i].profit;
            if(trade.PositionClose(posiciones[i].ticket))
            {
        ...
        // REGLA 3: Cerrar si ganancia > $3 pero se revirtió 30% desde su máximo
        else if(posiciones[i].profit >= 3.0 && 
                posiciones[i].profit < posiciones[i].maxProfitAlcanzado * 0.7)
        {
            double profitCierre = posiciones[i].profit;
            if(trade.PositionClose(posiciones[i].ticket))
            {
                ...
                gananciaAcumulada += profitCierre;
                SLconsecutivos = 0;
                Print("📉 GANANCIA PROTEGIDA: $", DoubleToString(profitCierre, 2), 
                      " (máx fue $", DoubleToString(posiciones[i].maxProfitAlcanzado, 2), ")");
            }
        }
    }
}
...
//+------------------------------------------------------------------+
//| TRAILING STOP MEJORADO - PARA GANANCIAS MÁS GRANDES             |
//+------------------------------------------------------------------+
void GestionarTrailingStopMejorado()
{
    ...
    for(int i = 0; i < ArraySize(posiciones); i++)
    {
        // Solo trailing si ganancia > $5 (no $1.5 como antes)
        if(posiciones[i].profit < 5.0) continue;
        ...
        double trailingPips = 10.0; // Base de 1 pip
        
        // Aumentar trailing según ganancia
        if(profitPorLote > 10.0) trailingPips = 20.0;  // 2 pips
        if(profitPorLote > 20.0) trailingPips = 30.0;  // 3 pips
        if(profitPorLote > 30.0) trailingPips = 50.0;  // 5 pips
        ...
        if(posiciones[i].tipo == ORDER_TYPE_BUY)
        {
            ...
            // Solo mover si mejora el SL en al menos 2 pips
            if(nuevoSL > sl + (2.0 * point * 10))
            {
                trade.PositionModify(posiciones[i].ticket, nuevoSL, 0);
                Print("🔼 TRAILING BUY: $", DoubleToString(posiciones[i].profit, 2), 
        ...
        else
        {
            ...
            if(nuevoSL < sl - (2.0 * point * 10) || sl == 0)
            {
                trade.PositionModify(posiciones[i].ticket, nuevoSL, 0);
                Print("🔽 TRAILING SELL: $", DoubleToString(posiciones[i].profit, 2), 
                      " | SL movido ", DoubleToString(trailingPips, 1), "p");
            }
        }
    }
...
//+------------------------------------------------------------------+
//| GESTIÓN DE CIERRE PARCIAL PARA BLOQUEAR GANANCIAS               |
//+------------------------------------------------------------------+
void GestionarCierreParcial()
{
    for(int i = 0; i < ArraySize(posiciones); i++)
    {
        // Cerrar 50% de la posición si ganancia > $10
        if(posiciones[i].profit >= 10.0 && posiciones[i].lote > LoteInicial * 2)
        {
            double loteCerrar = posiciones[i].lote * 0.5; // Cerrar la mitad
            loteCerrar = NormalizarLote(loteCerrar);
            
            // Asegurar que no cerramos todo
            if(loteCerrar >= posiciones[i].lote)
                loteCerrar = NormalizarLote(posiciones[i].lote * 0.5);
            
            // Cerrar parcialmente
            if(trade.PositionClose(posiciones[i].ticket, loteCerrar))
            {
                Print("⚖️ CIERRE PARCIAL: $", DoubleToString(posiciones[i].profit, 2), 
                      " | Cerrados ", DoubleToString(loteCerrar, 2), " lotes");
                
                // Actualizar la posición restante
                posiciones[i].lote -= loteCerrar;
                posiciones[i].profit *= 0.5; // Aproximar el profit restante
            }
        }
    }
}
...
void AbrirPosicion(ENUM_ORDER_TYPE tipo, double lote, double precio)
{
    ...
    double tpPips = TakeProfitPips; // Usar el TP fijo de 15 pips
