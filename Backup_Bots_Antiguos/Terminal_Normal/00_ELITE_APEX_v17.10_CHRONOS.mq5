...
int OnInit() {
   ...
   CrearPanel();
   EventSetTimer(1); // El HUD se actualiza cada segundo independientemente del mercado
...
void UpdateTrends() {
   ...
   dirMacro = (price > eM[0]) ? 1 : -1;
   dir1H = (price > e1[0]) ? 1 : -1;
...
void DrawFibo(double p100, double p0, bool bull) {
   ...
   ObjectMove(0, FIBO_NAME, 0, iTime(_Symbol, InpTF_Mid, bull ? iLowest(_Symbol,InpTF_Mid,MODE_LOW,curLkb,0) : iHighest(_Symbol,InpTF_Mid,MODE_HIGH,curLkb,0)), p100);
   ObjectMove(0, FIBO_NAME, 1, TimeCurrent(), p0); f23 = bull ? f0-(f0-f100)*0.236 : f0+(f100-f0)*0.236; f38 = bull ? f0-(f0-f100)*0.382 : f0+(f100-f0)*0.382; f61 = bull ? f0-(f0-f100)*0.618 : f0+(f100-f0)*0.618; f70 = bull ? f0-(f0-f100)*0.705 : f0+(f100-f0)*0.705; f78 = bull ? f0-(f0-f100)*0.786 : f0+(f100-f0)*0.786;
}
...
void MonitorRetracement() {
   ...
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK), bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   bool bull = (f0 > f100);
   if(bull && ask <= f61) p1_pierced = true;
   if(!bull && bid >= f61) p1_pierced = true;
   double c0=iClose(_Symbol,InpTF_Exec,0), o0=iOpen(_Symbol,InpTF_Exec,0); bool confirmed = bull ? (c0>o0) : (c0<o0);
   if(confirmed && p1_pierced) { if(!pX_done) { ExecuteMarket(bull, "OTE_CHRONOS"); pX_done=true; state=STATE_WAIT_BOS; SaveState(); } }
}
...
void ExecuteMarket(bool bull, string comment) {
   ...
   double sl = bull ? SymbolInfoDouble(_Symbol,SYMBOL_ASK) - InpSL_Pips*ps : SymbolInfoDouble(_Symbol,SYMBOL_BID) + InpSL_Pips*ps;
   double tp = bull ? SymbolInfoDouble(_Symbol,SYMBOL_ASK) + (InpSL_Pips*InpRR_Ratio)*ps : SymbolInfoDouble(_Symbol,SYMBOL_BID) - (InpSL_Pips*InpRR_Ratio)*ps;
...
void CrearPanel() {
   ObjectsDeleteAll(0, HUD_PRE); int x=15, y=30, w=355, h=isMinimized ? 45 : 640;
   CrRect("bg", x, y, w, h, C'18,10,10', clrMaroon, 2); CrRect("hdr", x+2, y+2, w-4, 40, clrMaroon, clrRed);
   CrLabel("ttl", x+15, y+8, "ELITE APEX v17.10 CHRONOS", clrWhite, 10);
   CrLabel("clk", x+w-120, y+8, "[00:00]", clrGold, 10);
   CrBtn("b_min", x+w-35, y+2, 25, 25, isMinimized ? "[+]" : "[-]", clrMaroon, clrWhite);
   if(!isMinimized) {
      CrRect("pl_bg", x+10, y+75, w-20, 105, clrBlack, clrMaroon); 
      CrLabel("pl_t", x+20, y+80, "🎯 ESTRATEGIA WARRIOR PREDATOR:", clrOrangeRed, 7);
      string b1 = (dirMacro == 1) ? "✔ BIAS ALCISTA: Esperando giro Sniper en zonas." : "✔ BIAS BAJISTA: Esperando giro Sniper en zonas.";
      CrLabel("planA", x+25, y+100, b1, clrWhite, 7); 
      CrLabel("vis", x+25, y+115, "VISIÓN: " + (string)curHours + "h | PROTECCIÓN: " + (string)curBE + "pts", clrGray, 7);
      CrBtn("b_day", x+10, y+190, 165, 30, "MODO DIARIO", currentPreset==PRESET_DIARIO?clrCyan:clrCrimson, currentPreset==PRESET_DIARIO?clrCrimson:clrWhite);
      CrBtn("b_we", x+180, y+190, 165, 30, "MODO FINDE", currentPreset==PRESET_FINDE?clrCyan:clrCrimson, currentPreset==PRESET_FINDE?clrCrimson:clrWhite);
      CrRect("ote_bg", x+10, y+230, w-20, 105, clrBlack, clrGold); 
      CrLabel("ote_t", x+20, y+235, "🏹 RED DE CAZA ACTIVADA:", clrGold, 7);
      CrLabel("oteX", x+25, y+255, "IMPULSO (23.6%): " + (f23>0?DoubleToString(f23,_Digits):"BUSCANDO..."), clrWhite, 7);
      CrLabel("ote0", x+25, y+268, "IMPULSO (38.2%): " + (f38>0?DoubleToString(f38,_Digits):"BUSCANDO..."), clrWhite, 7);
      CrLabel("ote1", x+25, y+281, "OTE 1 (61.8%): " + (f61>0?DoubleToString(f61,_Digits):"BUSCANDO..."), clrWhite, 7);
      CrLabel("ote2", x+25, y+294, "OTE 2 (70.5%): " + (f70>0?DoubleToString(f70,_Digits):"BUSCANDO..."), clrWhite, 7);
      CrLabel("ote3", x+25, y+307, "OTE 3 (78.6%): " + (f78>0?DoubleToString(f78,_Digits):"BUSCANDO..."), clrWhite, 7);
      CrLabel("stV", x+15, y+345, botStatus, clrWhite, 9); 
      CrLabel("tr4", x+15, y+365, "BIAS MACRO (1H): " + (dirMacro==1?"ALCISTA ▲":"BAJISTA ▼"), (dirMacro==1?clrCyan:clrOrangeRed), 8);
      CrLabel("tr1", x+15, y+385, "ESTRUCTURA (M15): " + (dir1H==1?"ALCISTA ▲":"BAJISTA ▼"), (dir1H==1?clrCyan:clrOrangeRed), 8);
      CrBtn("b_zen", x+10, y+425, 165, 30, "MODO ZEN", currentMode==MODE_ZEN?clrCyan:clrCrimson, currentMode==MODE_ZEN?clrCrimson:clrWhite);
      CrBtn("b_har", x+180, y+425, 165, 30, "COSECHA", currentMode==MODE_COSECHA?clrCyan:clrCrimson, currentMode==MODE_COSECHA?clrCrimson:clrWhite);
      CrBtn("b_mar", x+10, y+465, 165, 30, "MERCADO (Gatillo)", currentExec==EXEC_MARKET?clrCyan:clrCrimson, currentExec==EXEC_MARKET?clrCrimson:clrWhite);
      CrBtn("b_lim", x+180, y+465, 165, 30, "AUTO (Zonas)", currentExec==EXEC_LIMIT?clrCyan:clrCrimson, currentExec==EXEC_LIMIT?clrCrimson:clrWhite);
      CrBtn("b_c", x+10, y+505, 108, 30, "COMPRA", currentDir==DIR_COMPRAS?clrCyan:clrCrimson, currentDir==DIR_COMPRAS?clrCrimson:clrWhite);
      CrBtn("b_a", x+123, y+505, 108, 30, "AMBAS", currentDir==DIR_AMBAS?clrCyan:clrCrimson, currentDir==DIR_AMBAS?clrCrimson:clrWhite);
      CrBtn("b_v", x+236, y+505, 108, 30, "VENTA", currentDir==DIR_VENTAS?clrCyan:clrCrimson, currentDir==DIR_VENTAS?clrCrimson:clrWhite);
      CrBtn("b_cl", x+10, y+555, 335, 60, "CERRAR TODO (PANIC)", clrRed, clrWhite);
   }
}
void UpdatePanelText() {
   if(isMinimized) return;
   // ESTIMACIÓN DE SERVER TIME PARA RELOJ FLUIDO SIN TICKS
   datetime currentServerEst = TimeCurrent();
   if(lastTickLocal > 0) currentServerEst = lastTickServer + (datetime)(TimeLocal() - lastTickLocal);
   long sec = iTime(_Symbol, InpTF_Exec, 0) + PeriodSeconds(InpTF_Exec) - currentServerEst; if(sec < 0) sec = 0;
   string clk = StringFormat("[%02d:%02d]", sec/60, sec%60);
   ObjectSetString(0,HUD_PRE+"clk",OBJPROP_TEXT,clk);
   UpdateTrends();
   ObjectSetString(0,HUD_PRE+"oteX",OBJPROP_TEXT,"IMPULSO (23.6%): " + (f23>0?DoubleToString(f23,_Digits):"BUSCANDO..."));
   ObjectSetString(0,HUD_PRE+"ote0",OBJPROP_TEXT,"IMPULSO (38.2%): " + (f38>0?DoubleToString(f38,_Digits):"BUSCANDO..."));
   ObjectSetString(0,HUD_PRE+"ote1",OBJPROP_TEXT,"OTE 1 (61.8%): " + (f61>0?DoubleToString(f61,_Digits):"BUSCANDO..."));
   ObjectSetString(0,HUD_PRE+"ote2",OBJPROP_TEXT,"OTE 2 (70.5%): " + (f70>0?DoubleToString(f70,_Digits):"BUSCANDO..."));
   ObjectSetString(0,HUD_PRE+"ote3",OBJPROP_TEXT,"OTE 3 (78.6%): " + (f78>0?DoubleToString(f78,_Digits):"BUSCANDO...")); 
