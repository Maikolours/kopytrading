//+------------------------------------------------------------------+
//|                ELITE GOLD MAIKO SNIPER | HYBRID PRO             |
//|      "DYNAMIC SOS PROGRESSION" | VERSION 14.00 (HIBRIDO)        |
//+------------------------------------------------------------------+
...
input bool UsarHorario = true;
input int HoraInicio = 1; // Hora de inicio (broker)
input int HoraFin = 23;   // Hora de fin (broker)
input bool BloquearViernesNoche = true; // Bloquea entradas nuevas viernes tarde
...
input double MultiplicadorRefuerzo = 1.5; // AGRESIVO: 1.5x lotaje contra
...
input bool UsarModoScalp = false;        // [HIBRIDO = FALSE] NO Cierra posiciones individuales
input double ProfitScalpIndividual = 1.50; // Profit individual para posiciones SOS ($)
input double ProfitScalpMinLote = 0.75;    // Profit individual para posiciones de 0.01 ($)
input bool UsarEmergenciaAuto = true;    // Activa la salida de emergencia inteligente
input double ProfitEmergenciaUSD = 1.00;  // TP de emergencia global ($)
input double ProfitNetoUSD = 1.00;   
input double ProfitNetoCesta = 1.00;    // Profit mínimo si hay 2+ operaciones ($)
input double LimiteDiario = 250.0;      // Límite de beneficio diario ($)
input double DistanciaRefuerzoPipsBase = 10.0; // Distancia para iniciar SOS
...
input string TradeComment = "MAIKO_HYBRID_PRO";
...
int hEMA_H4, hEMA_H1, hEMA_M15, hEMA_M5, hEMA_M1, hEMA_VisualM5, hRSI, hMACD, hATR, hADX, hADX_Chart;
int hRadar[7];
ENUM_TIMEFRAMES etfs[]={PERIOD_W1,PERIOD_D1,PERIOD_H4,PERIOD_H1,PERIOD_M15,PERIOD_M5,PERIOD_M1};
bool indicadoresVisualesOK = false;
...
int OnInit() {
    ...
    hEMA_VisualM5 = iMA(_Symbol, PERIOD_M1, 250, 0, MODE_EMA, PRICE_CLOSE);
...
void OnTimer() {
    ...
    if(!indicadoresVisualesOK) {
        ...
        bool hasM1 = false, hasM5 = false;
        for(int i=0; i<numInd; i++) {
            string n = ChartIndicatorName(0, 0, i);
            if(StringFind(n, "50") >= 0) hasM1 = true;
            if(StringFind(n, "250") >= 0) hasM5 = true;
        }
        bool ok = true;
        if(!hasM1) {
            if(!ChartIndicatorAdd(0, 0, hEMA_M1)) {
                Print("Error adding hEMA_M1: ", GetLastError());
                ok = false;
            }
        }
        if(!hasM5) {
            if(!ChartIndicatorAdd(0, 0, hEMA_VisualM5)) {
                Print("Error adding hEMA_VisualM5: ", GetLastError());
                ok = false;
            }
        }
        if(ok && winRSI != -1 && winMACD != -1 && winADX != -1) indicadoresVisualesOK = true;
    ...
    flotante = CalcularProfit();
    if(UsarHorario) {
        MqlDateTime dt; TimeLocal(dt); // Use TimeLocal for weekends when server time stops!
        if(dt.day_of_week == 0 || dt.day_of_week == 6) {
            txtVeredicto = "MERCADO CERRADO (FIN DE SEMANA) 🕒";
            txtVoz = "MERCADO CERRADO.";
        }
        else if(ArraySize(pos) == 0) {
            if(dt.hour < HoraInicio || dt.hour >= HoraFin) {
                txtVeredicto = "MERCADO CERRADO / ESPERANDO 🕒";
                txtVoz = "FUERA DE HORARIO.";
            }
            else if(BloquearViernesNoche && dt.day_of_week == 5 && dt.hour >= 20) {
                txtVeredicto = "VIERNES NOCHE: BLOQUEADO 🛑";
                txtVoz = "FIN DE SEMANA ANTICIPADO.";
            }
        }
    }
    ActualizarInterfazMaster();
...
void OnTick() {
    ...
    double baseTarget = (ArraySize(pos) >= 2) ? ProfitNetoCesta : ProfitNetoUSD;
    double profitTarget = (CUENTA_REAL_CENT ? (baseTarget * 100.0) : baseTarget);
    ...
    // GestionarCosechaSniper(); // DESACTIVADO EN MODO HÍBRIDO (Se cierra todo en cesta global)
    ActualizarRadarMaster();
    
    if(!BotActivo) { txtVoz = "SISTEMA EN PAUSA."; }
    else if(bloqueadoPorNoticia) { txtVoz = "PAUSA POR NOTICIAS."; }
    else { txtVoz = "ANALIZANDO MERCADO..."; }
    
    ActualizarInterfazMaster();

    if(!BotActivo) return;
    if(bloqueadoPorNoticia) return;
    ...
    if(ArraySize(pos) > 0) { 
        ...
        if(UsarHorario) {
            ...
            if(BloquearViernesNoche && dt.day_of_week == 5 && dt.hour >= 20) {
                txtVeredicto = "VIERNES NOCHE: BLOQUEADO 🛑";
                txtVoz = "FIN DE SEMANA ANTICIPADO.";
                return;
            }
        ...
        txtVoz = (direccion == 1) ? "BUSCANDO COMPRA..." : ((direccion == -1) ? "BUSCANDO VENTA..." : "ESPERANDO TENDENCIA CLARA...");
        
        bool emaOK = (direccion == 1 && bid > emaM5[0] && bid > emaM1[0]) || (direccion == -1 && bid < emaM5[0] && bid < emaM1[0]);
        
        if(!emaOK) txtVeredicto = "ESPERANDO ROTURA DE EMAS...";
...
void GestionarModoCascada(int direccion, double emaM1) {
    ...
    if(direccion != 0 && ((pos[0].t == POSITION_TYPE_BUY && direccion == -1) || (pos[0].t == POSITION_TYPE_SELL && direccion == 1))) {
        ...
        if(pos[0].t == POSITION_TYPE_BUY) trade.Buy(lotCascada, _Symbol, 0, 0, 0, TradeComment + "_CASCADA_HIBRIDA");
        else trade.Sell(lotCascada, _Symbol, 0, 0, 0, TradeComment + "_CASCADA_HIBRIDA");
        ultimoCascada = TimeCurrent();
        txtVeredicto = "CASCADA HIBRIDA 🔥";
    }
}
...
void GestionarRefuerzoInteligente(double distSOS, double prExtrema, double distContActual) {
    ...
    if(!EsperarGiroM1_SOS) {
        double lotesContra = 0;
        for(int i=0; i<ArraySize(pos); i++) lotesContra += pos[i].v;
        double volRef = NormalizeDouble(lotesContra * MultiplicadorRefuerzo, 2);
        if(volRef < LotajeMinimo) volRef = LotajeMinimo;
        if(volRef > MaxLoteIndividual) volRef = MaxLoteIndividual;
        if(volTotal + volRef > MaxLoteTotal) volRef = NormalizeDouble(MaxLoteTotal - volTotal, 2); 
        if(volRef >= 0.01) {
            if(pos[0].t == POSITION_TYPE_BUY) trade.Buy(volRef, _Symbol, 0, 0, 0, TradeComment + "_SOS_HIBRIDO");
            else trade.Sell(volRef, _Symbol, 0, 0, 0, TradeComment + "_SOS_HIBRIDO");
            ultimoSOS = TimeCurrent();
            txtVeredicto = "RESCATE HIBRIDO AL TOQUE 🛡️⚡";
        }
        return;
    }
    ...
    if(((pos[0].t == POSITION_TYPE_BUY && pat == 2) || (pos[0].t == POSITION_TYPE_SELL && pat == -2)) && colorActualOK) {
        ...
        if(volRef >= 0.01) {
            ...
            ultimoSOS = TimeCurrent();
            txtVeredicto = "RESCATE HIBRIDO DINAMICO 🛡️⚡";
        }
...
void CrearInterfazMaster() { 
    ...
    CrearLabel("MAIKO_T", x+10, y+10, "MAIKO PRO | HYBRID GOLD v14.0", ColorMain, 11, 10001); 
