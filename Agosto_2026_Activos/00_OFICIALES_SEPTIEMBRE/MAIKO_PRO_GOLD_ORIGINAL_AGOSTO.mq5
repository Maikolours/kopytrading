//+------------------------------------------------------------------+
//|       MAIKO PRO GOLD | ORIGINAL AGOSTO                          |
...
input double   MaxDistanciaEmaPips        = 50.0;        // 📏 Distancia Máxima a la EMA (Pips) Evita entrar al final
...
input bool     UsarHorarioBloqueo         = false;       // 🛑 Evitar Noticias (Bloqueo Horario desactivado de serie)
...
input int      LimitePosicionesSOS        = 1;           // 🛡️ Límite Posiciones SOS (1 = Máx 2 en total)
...
input bool     UsarStopLossDinero         = true;        // 🚨 Activar Stop Loss en Dinero ($)
input double   StopLossDinero             = 50.0;        // 🚨 Pérdida Máxima por Cesta ($)
...
input int      MinutosPausaTrasStopLoss   = 20;          // ⏳ Minutos de Pausa tras Stop Loss
...
string         HUD_Branding               = "MAIKO PRO GOLD DEMO";
...
void OnTick() {
    ...
    bool slAlcanzado = false;
    if(ArraySize(pos) > 0) {
        if(UsarStopLossDinero && flotante <= -(MathAbs(StopLossDinero) * multCent)) {
            slAlcanzado = true;
        } else if(UsarStopLossPorcentaje) {
            double balance = AccountInfoDouble(ACCOUNT_BALANCE);
            if(balance > 0) {
                double maxLossAllowed = balance * (MathAbs(PorcentajeStopLoss) / 100.0);
                if(flotante <= -maxLossAllowed) slAlcanzado = true;
            }
        }
    }
