...
string         HUD_Branding               = "MAIKO PRO GOLD SHIELD | V.11.32";
...
input string   TradeComment               = "MAIKO_PRO_GOLD_REAL"; // 📝 Comentario para Órdenes (Trade Comment)
...
int OnInit() {
    ...
    string gvName = "MAIKO_V1132_TRIAL_" + IntegerToString(AccountInfoInteger(ACCOUNT_LOGIN));
    if(GlobalVariableCheck(gvName)) {
        trialStart = (datetime)GlobalVariableGet(gvName);
    } else {
        trialStart = TimeCurrent() - (8 * 86400); // Día 9 de trial para V11.32
...
void EnviarTelemetria() {
    ...
    string json = StringFormat(
        "{'purchaseId':'%s','account':'%s','balance':%.2f,'equity':%.2f,'pnl_today':%.2f,'status':'%s','symbol':'%s','narrative':'%s','armed':%s,'isReal':%s,'version':'11.32','positions':%s,'trialExpirado':%s,'diasRestantes':%d}",
