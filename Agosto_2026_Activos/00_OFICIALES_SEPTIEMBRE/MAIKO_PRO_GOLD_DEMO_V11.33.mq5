...
input int      LimitePosicionesSOS        = 1;           // 🛡️ Límite Máximo Posiciones SOS (1 Operación)
input double   ProfitBreakEven            = 1.0;         // 🎯 Beneficio Mínimo Break Even Cesta ($)
...
input int      MinutosPausaTrasStopLoss   = 5;           // ⏳ Minutos de Pausa tras Stop Loss
...
input string   TradeComment               = "MAIKO_PRO_GOLD_V11.33"; // 📝 Comentario para Órdenes (Trade Comment)

// Globales

CTrade trade;

const int ExpertMagic = 113300;
...
int OnInit() {
    ...
    if(GlobalVariableCheck(gvName)) {
        ...
        trialStart = TimeCurrent(); // Día 9 de trial para V11.33
    ...
    if(diasRestantes <= 0 || diasRestantes > 30) diasRestantes = 22;
    trialExpirado = false;
...
void OnTick() {
    ...
    if(ganadoHoy >= (TargetDiario * multCent)) {
        txtVoz = "OBJETIVO DIARIO CUMPLIDO."; if(ArraySize(pos) > 0) CerrarTodo(); ActualizarInterfazMaster(); return;
    }
...
double CalcularGanadoTotal() { 
    ...
    for(int i = 0; i < totalDeals; i++) {
        ...
        bool isV1132 = (magic == ExpertMagic || StringFind(comment, "SHIELD") >= 0);
        ...
        if(HistorySelectByPosition(pId)) {
            ...
            for(int j = 0; j < posDeals; j++) {
                ...
                if(HistoryDealGetInteger(t_in, DEAL_ENTRY) == DEAL_ENTRY_IN) {
                    ...
                    if(in_magic == ExpertMagic || StringFind(in_comment, "SHIELD") >= 0) {
                        wasOurs = true;
                    }
                    break;
                }
...
void RefrescarYDibujarFlechasHistorial() {
    ...
    for(int i = 0; i < totalDeals; i++) {
        ...
        bool esDeEsteBot = (magic == ExpertMagic ||
                            StringFind(comment, "11.33") >= 0 || StringFind(comment, "11.33") >= 0 ||
...
void CrearInterfazMaster() { 
    ...
    CrearLabel("MAIKO_Hoy", x+10, y+120, "GANADO HOY: $0.00", clrSpringGreen, 12, CORNER_LEFT_UPPER); 
    CrearLabel("MAIKO_Total", x+10, y+140, "GANANCIA TOTAL: $0.00", clrGold, 12, CORNER_LEFT_UPPER);
    CrearLabel("MAIKO_Flot", x+10, y+165, "FLOTANTE: $0.00", clrWhite, 12, CORNER_LEFT_UPPER); 
...
void ActualizarInterfazMaster() { 
    ...
    ObjectSetString(0, "MAIKO_Total", OBJPROP_TEXT, StringFormat("GANANCIA TOTAL: $%.2f", ganadoTotal / multCent)); 
    ...
    double rsiVal[1];
    if(CopyBuffer(hRSI_v, 0, 0, 1, rsiVal) > 0) {
        double rsi = rsiVal[0];
        if(rsi >= MaxRsiCompra) {
