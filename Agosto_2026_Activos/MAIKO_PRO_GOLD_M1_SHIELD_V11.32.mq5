...
string         HUD_Branding               = "MAIKO PRO GOLD SHIELD";
...
input int      ExpertMagic                = 111333;                   // 🔢 Magic Number del Bot
...
int OnInit() {
    ...
    // Si está en cuenta Real o Gold, es OBLIGATORIO tener clave de licencia
    if(!isDemoAccount && !MQLInfoInteger(MQL_TESTER)) {
        string lic = MiLicencia;
        StringTrimLeft(lic);
        StringTrimRight(lic);
        if(lic == "") {
            Alert("MAIKO PRO GOLD: Para operar en cuenta REAL es OBLIGATORIO introducir tu Clave de Licencia o ID Vínculo en los parámetros (F7).");
            BotActivo = false;
