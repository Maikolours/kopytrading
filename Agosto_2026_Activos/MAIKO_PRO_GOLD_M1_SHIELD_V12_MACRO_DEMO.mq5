...
//|            ELITE GOLD MAIKO SNIPER V12.00 | NORMAL HISTORICO     |
...
input double   MaxRangoVelaM1             = 40.0;        // ⚡ Rango Máximo Vela M1 (Pips)
...
input double   UmbralADXTendencia         = 45.0;        // 📊 Nivel ADX para Considerar Tendencia Fuerte
...
string         HUD_Branding               = "MAIKO V12 | HIBRIDO DEMO";
...
input string   TradeComment               = "V12_SHIELD_DEMO";        // 🏷️ Comentario para órdenes (Trade Comment)


// Globales

CTrade trade;
const int ExpertMagic = 999999;
...
void ActualizarTextosEstado() {
    ...
    if(!enHorario) {
        ...
        txtVoz = "SCHOLAR: Analizando Accion del Precio";
    }
...
void AnalizarMacroContexto() {
    ...
    if(adx[0] > UmbralADXTendencia) {
        ...
        txtMacro = StringFormat("MERCADO LATERAL (ADX DEBIL: %.1f)", adx[0]);
...
void OnTick() {
    ...
    if(UsarTrailingStop) GestionarTrailingStopTendencia();
     ActualizarRadarMaster(); ActualizarInterfazMaster();
...
bool ValidarEstructuraScholar(string &decision) {
    ...
    if(modoTendencia) {
        ...
        if(trend15_UP && trend5_UP) { 
            ...
            txtVeredicto = "ESPERANDO ALINEACION M15/M5 (TENDENCIA)"; return false;
        }
    } else {
        // MODO LATERAL (PING-PONG)
        double bUp[1], bDn[1];
        int hBands = iBands(_Symbol, PERIOD_M15, 20, 0, 2.0, PRICE_CLOSE);
        CopyBuffer(hBands, 1, 1, 1, bUp); 
        CopyBuffer(hBands, 2, 1, 1, bDn); 
        
        double anchuraPips = (bUp[0] - bDn[0]) / _Point / 10;
        if(anchuraPips < AnchoMinimoEmbudo) {
            txtVeredicto = StringFormat("MERCADO BASURA (EMBUDO %.1f PIPS)", anchuraPips); return false;
        }
        
        double rsi[1];
        if(CopyBuffer(hRSI_v, 0, 1, 1, rsi) <= 0) return false;
        
        double prevPrecio = iClose(_Symbol, PERIOD_M15, 2);
        double bMid = (bUp[0] + bDn[0]) / 2.0;

        

        if(precio < bMid) {
            if(prevPrecio > precio && rsi[0] < 50) {
                decision = "SELL";
            } else if (prevPrecio <= precio) {
                txtVeredicto = "ESPERANDO: PRECIO NO ESTA CAYENDO"; return false;
            } else {
                txtVeredicto = StringFormat("ESPERANDO: RSI (%.1f) NO ES BAJISTA (<50)", rsi[0]); return false;
            }
        }
        else if(precio > bMid) {
            if(prevPrecio < precio && rsi[0] > 50) {
                ...
                txtVeredicto = StringFormat("ESPERANDO: RSI (%.1f) NO ES ALCISTA (>50)", rsi[0]); return false;
            }
        } else {
            txtVeredicto = "ESPERANDO MICRO-TENDENCIA M15"; return false;
        }
    }
...
void ActualizarInterfazMaster() { 
    ...
    if(metaTP > 0) {
        ...
        ObjectSetString(0, "MAIKO_MetaTP", OBJPROP_TEXT, "ESTADO: " + (surfeandoTendencia ? "BUSCANDO PULLBACKS" : "ESPERANDO TENDENCIA MACRO"));
...
void EnviarTelemetria() {
    ...
    string json = StringFormat(
        ...
        "\"armed\":%s,\"isReal\":%s,\"version\":\"12.00\",\"positions\":%s,"
