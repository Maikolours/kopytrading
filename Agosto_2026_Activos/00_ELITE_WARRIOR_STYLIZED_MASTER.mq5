...
input int             InpBE_Offset_Pts = 100; // Puntos a asegurar en BE (100 = $1.00)
...
void ManageRisk() {
   ...
   for(int i=PositionsTotal()-1; i>=0; i--) if(posInfo.SelectByIndex(i) && posInfo.Magic()==InpMagic) {
      ...
      if(pnl >= curBE && (sl==0 || (buy && sl<op) || (!buy && sl>op))) trade.PositionModify(posInfo.Ticket(), op + (buy ? InpBE_Offset_Pts : -InpBE_Offset_Pts)*ps, tp);
