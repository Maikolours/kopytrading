//+------------------------------------------------------------------+
//|     KOPYTRADE_BTCUSD_EVOLUTION_v7_4_2 - BTC MASTER BLINDADO      |
...
#property strict
#property description "BTC MASTER BLINDADO | Full Lock Hedge | History PnL HUD"
      ...
      if(posInfo.SelectByIndex(i) && posInfo.Magic()==activeMagic) {
         ...
         if(ActivarBE && pUSD >= BE_Trigger_USD) {
            ...
            if(posInfo.PositionType()==POSITION_TYPE_BUY) {
               double be=posInfo.PriceOpen()+100*_Point; // Higher distance for BTC
...
void ManageStrike() {
   ...
   if(posTotal == 0) {
      DeletePendings();
      if(biasUp && (currentDir==DIR_AMBAS || currentDir==DIR_SOLO_COMPRAS)) trade.Buy(LoteManual,_Symbol,ask,0,0,commentTag);
      else if(biasDn && (currentDir==DIR_AMBAS || currentDir==DIR_SOLO_VENTAS)) trade.Sell(LoteManual,_Symbol,bid,0,0,commentTag);
      return;
   }
   ...
   if(!hasOpposite && pUSD <= -GatilloHedge_USD) {
      ...
      if(!hasPending) {
         ...
         double dist = (_Symbol=="XAUUSD" || _Symbol=="XAUUSD-VIP") ? 300 : 5000; // Point distance for BTC vs Gold
         double targetHedgeP = (mainType==POSITION_TYPE_BUY) ? entryP - dist*_Point : entryP + dist*_Point;
...
void CrearHUD() {
   ...
   CrLabel("ttl",x+15,y+10,"BTC EVO v7.4.2 BLINDADO",clrWhite,10,"Arial Bold");
