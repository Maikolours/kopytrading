           ...
           if(p < 0 && MathAbs(p) >= (eff_StopIndiv * 0.4) && posInfo.Comment() != "RESCATE_P") {
              ...
              if(activeRescues == 0) {
                 ...
                 if(posInfo.PositionType() == POSITION_TYPE_BUY) {
                    ...
                    double resTP = bid - (tpPoints * _Point);
                    if(trade.Sell(resLot, _Symbol, bid, 0, NormalizeDouble(resTP, _Digits), "RESCATE_P"))
                       Print("PHASE 3: Rescate SELL lanzado...");
                 } else {
                    // Si el principal es SELL, el rescate es BUY
                    double resTP = ask + (tpPoints * _Point);
                    if(trade.Buy(resLot, _Symbol, ask, 0, NormalizeDouble(resTP, _Digits), "RESCATE_P"))
      ...
      if(netProfit > (eff_HarvestTP * 0.4)) { // Bajamos el umbral para que entre antes
         ...
         // Ajustar orden si el precio se aleja demasiado (Chase)
            ...
            if(OrderSelect(t_ref)) {
               ...
               if(mainType==POSITION_TYPE_BUY && ask+dRef < p_ref - 50*_Point) trade.OrderModify(t_ref, NormalizeDouble(ask+dRef, _Digits), 0, 0, ORDER_TIME_GTC, 0, 0);
               if(mainType==POSITION_TYPE_SELL && bid-dRef > p_ref + 50*_Point) trade.OrderModify(t_ref, NormalizeDouble(bid-dRef, _Digits), 0, 0, ORDER_TIME_GTC, 0, 0);
            }
         }
