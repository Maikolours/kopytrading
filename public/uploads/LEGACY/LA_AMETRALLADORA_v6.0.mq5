//+------------------------------------------------------------------+
//|          LA_AMETRALLADORA_v6.0 - EDICIÓN "TENDENCIA"             |
//|   FILTRO DE MEDIA MÓVIL 200 + ENCADENAMIENTO + LÍMITES SEGUROS   |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, Kopytrade Corp."
#property link      "https://www.kopytrade.com"
#property version   "6.00"
#property strict
#property description "La Ametralladora v6.0 | Filtro de Tendencia SMA200 | Kopytrade"
...
//============================================================
input group "=== SEGURIDAD & DASHBOARD ==="
input string   LicenseKey        = "TRIAL-2026";
input string   PurchaseID        = "";         // ID de Vínculo (Dashboard kopytrading.com)
...
input double   LoteManual         = 0.01;      // Lote Inicial de Operación
input int      MagicNumber        = 606060;    // Numero Identificador
input double   MaxDrawdown_USD    = 500.0;     // 🛑 Stop Emergencia Cuenta (unid.)
input double   Max_DD_Individual  = 100.0;     // 🛑 Stop por Operación (unid.)
input int      MaxPosiciones      = 4;         // 📈 Máximo de posiciones abiertas
input int      Max_Velas_Vida     = 5;         // ⏳ Tiempo máximo de vida velas
...
input double   DistanciaRescate   = 25.0;      // $ Perdida para activar gatillo (unid)
...
input double   MetaDiaria_USD      = 500.0;     // 🎯 Ganancia Diaria (unid.)
input double   Meta_Ciclo_USD      = 100.0;     // Neto para cerrar ciclo (unid)
input double   Harvest_TP_USD      = 12.0;      // Meta por operación individual (unid)
...
input double   BE_Trigger_USD      = 20.0;      // Activar BE tras (unidades)
...
input int      MomentumCandles     = 3;         // Velas para medir fuerza
input int      MomentumRequired    = 2;         // Cuantas velas deben coincidir
input int      CooldownSeconds     = 30;        // Espera tras cierre (seg)
...
double         mode_HarvestTP = 12.0;
double         mode_CycleMeta = 100.0;
double         mode_BETrigger = 20.0;

#define PNL "AMETRA_v6_"
...
int OnInit() {
   ...
   if(CuentasPermitidas != "") {
      ...
      if(StringFind(CuentasPermitidas, login) == -1) {
         Alert("ERROR: Esta cuenta ("+login+") no está autorizada.");
         return(INIT_FAILED);
      }
   }
...
void OnTick() {
   ...
   if(goalReached) {
      if(PositionsTotalBots() == 0) {
         ...
         DeleteBotPendings(); ActualizarPanel(); return;
      }
      if(currentNet >= 0) {
...
void MaintainGates() {
   ...
   if(botPosCount > 0) {
      ...
      // --- LOGICA DE REFUERZO (SOLO SI HAY GANANCIA DE 50 unid) ---
      ...
      if(netProfit > 50.0) {
         if(t_ref == 0) {
            if(mainType == POSITION_TYPE_BUY) trade.BuyStop(LoteManual, targetRef, _Symbol, 0, 0, 0, 0, "REFUERZO");
            else trade.SellStop(LoteManual, targetRef, _Symbol, 0, 0, 0, 0, "REFUERZO");
         }
   ...
   if(botPosCount == 0) {
      ...
      if((currentDir == DIR_COMPRAS || currentDir == DIR_AMBAS) && p_buys == 0 && allowBuy)
         trade.BuyStop(LoteManual, SymbolInfoDouble(_Symbol, SYMBOL_ASK) + dist, _Symbol, 0, 0, 0, 0, "G_BUY");
      if((currentDir == DIR_VENTAS || currentDir == DIR_AMBAS) && p_sells == 0 && allowSell)
         trade.SellStop(LoteManual, SymbolInfoDouble(_Symbol, SYMBOL_BID) - dist, _Symbol, 0, 0, 0, 0, "G_SELL");
   }
}
    ...
    for(int i=PositionsTotal()-1; i>=0; i--) {
       if(posInfo.SelectByIndex(i) && posInfo.Magic() == MagicNumber) {
          ...
          if(p >= mode_HarvestTP) { 
             ...
             if(posInfo.Comment() == "RESCATE_P") coolingEndTime = 0; // CHAINING
             continue; 
          }
...
double GetCurrentNetProfit() {
   double p = 0;
   for(int i=PositionsTotal()-1; i>=0; i--) if(posInfo.SelectByIndex(i) && posInfo.Magic() == MagicNumber) p += posInfo.Profit() + posInfo.Commission() + posInfo.Swap();
   return p;
}
...
void CrearPanel() {
   ...
   int x=15, y=15, w=295, h=390;
   if(isMinimized) { w=220; h=65; }
   CrRect("bg", x, y, w, h, CLR_BG, CLR_BRD, 2);
   CrRect("hdr", x+2, y+2, w-4, 40, CLR_HDR, CLR_HDR);
   CrLabel("ttl", x+15, y+8, "AMETRALLADORA v6.0", clrWhite, 11, "Arial Bold");
   CrLabel("sub", x+15, y+25, "Filtro Trend SMA200", CLR_MUTED, 7);
   CrBtn("min", x+w-25, y+8, 18, 18, isMinimized?"+":"-", CLR_HDR, clrWhite);
   
   if(!isMinimized) {
      CrLabel("licV", x+15, y+60, "LICENCIA: " + LicenseKey, CLR_WARN, 8, "Arial Bold");
      CrLabel("trL", x+15, y+90, "TENDENCIA:", CLR_MUTED, 8); 
      CrLabel("trV", x+110, y+90, trendStatus, (trendStatus=="ALCISTA"?CLR_SUCCESS:(trendStatus=="BAJISTA"?CLR_DANGER:CLR_WARN)), 9, "Arial Bold");
      
      CrLabel("pL", x+15, y+115, "PnL HOY:", CLR_MUTED, 8); CrLabel("pV", x+110, y+115, "0", CLR_TXT, 10, "Arial Bold");
      CrLabel("stL", x+15, y+140, "ESTADO:", CLR_MUTED, 8); CrLabel("stV", x+110, y+140, botStatus, CLR_SUCCESS, 9);
      
      CrLabel("moH", x+12, y+175, "MODOS & DIRECCIÓN", CLR_MUTED, 7);
      ...
      CrBtn("b_close", x+10, y+270, 275, 30, "CERRAR TODO (EMERGENCIA)", CLR_DANGER, clrWhite);
      
      string rem = "REMOTO: " + (remotePaused?"🔴 PAUSA":"🟢 ONLINE");
      CrLabel("rem", x+15, y+325, rem, (remotePaused?CLR_DANGER:CLR_SUCCESS), 8);
   }
   ChartRedraw(0);
}
...
bool CheckLicenseServer() {
   if(PurchaseID == "") {
      ...
      if(AccountInfoInteger(ACCOUNT_TRADE_MODE) != ACCOUNT_TRADE_MODE_DEMO) {
         Alert("ERROR: El modo offline solo es para cuentas DEMO. Introduce tu PurchaseID.");
         return false;
      }
      return true;
   }
   ...
   if(res == 200) {
      ...
      if(StringFind(response, "\"allowed\":true") != -1) {
         Print("LICENSE: Validacion exitosa.");
         return true;
      } else {
         ...
         Alert("ERROR LICENCIA: " + msg);
         return false;
      }
   }
   
   Print("LICENSE: Error de conexion (" + IntegerToString(res) + "). Reintentando en Demo...");
   if(AccountInfoInteger(ACCOUNT_TRADE_MODE) != ACCOUNT_TRADE_MODE_DEMO) {
      Alert("ERROR: No se pudo verificar la licencia en cuenta REAL. Revisa tu internet.");
      return false;
   }
   return true;
}
