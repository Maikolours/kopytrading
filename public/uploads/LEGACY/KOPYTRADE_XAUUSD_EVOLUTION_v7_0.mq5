//+------------------------------------------------------------------+
//|     KOPYTRADE_XAUUSD_EVOLUTION_v7_0 - HYBRID MASTER BOT          |
//|   v5.55 SPEED + v6.10 SAFETY + MOBILE HUD + AUTO-VISUALS         |
...
#property strict
#property description "HYBRID MASTER BOT | v5.55 Speed + v6.10 Safety | Ultra Responsive"
...
int CustomBarShift(string symbol, ENUM_TIMEFRAMES timeframe, datetime time, bool exact=false) {
   ...
   if(count <= 0) return -1;
   for(int i = 0; i < count; i++) if(history_times[count-1-i] <= time) return i;
   return -1;
}
...
sinput string separator1 = "=========================="; // === CONFIGURACIÓN INTERNA ===
enum ENUM_MODE { MODE_ZEN, MODE_COSECHA };
enum ENUM_DIR  { DIR_AMBAS, DIR_SOLO_COMPRAS, DIR_SOLO_VENTAS };

input ENUM_MODE ModePreset        = MODE_COSECHA; // Modo inicial
input int      MagicNumber        = 700700;       // Magic Unique
input int      MaxPosiciones      = 3;            
input double   LoteManual         = 0.02;         
input double   MaxDrawdown_USD    = 50.0;         // Stop Emergencia Cuenta
input double   Max_DD_Individual_USD = 25.0;      // Stop por Operación

sinput string separator2 = "=========================="; // === TÁCTICA AMETRALLADORA ===
input int      DistanciaGates      = 300;         // Distancia entrada (en puntos)
input double   GatilloRescate_USD = 5.0;          // $ Pérdida para activar rescate
input double   Target_Harvest_USD = 1.0;          // Meta por operación (v5.55 style)
input double   Target_Rescue_USD  = 2.0;          // Meta por rescate (Limpieza)
input double   Net_Cycle_USD      = 5.0;          // Meta del clúster (Cierre total)

sinput string separator3 = "=========================="; // === PROTECCIÓN AVANZADA ===
input bool     ActivarBE           = true;        
input double   BE_Trigger_USD      = 2.0;         
input bool     ActivarTrailing     = true;        
input int      TrailingPoints      = 1500;        
input int      TrailingStep        = 500;         

sinput string separator4 = "=========================="; // === VISUALES & HORARIO ===
input bool     AutoDrawIndicators  = true;         // Auto-dibujar EMAS/RSI
...
int OnInit() {
   ...
   CrearHUD();
   remotePaused = false; // Reset security on init
   EventSetTimer(3);
...
void OnTick() {
   ...
   if(MaxDrawdown_USD > 0 && netUSD <= -MaxDrawdown_USD) {
      CloseAll(); DeletePendings(); botStatus = "🛑 STOP EMERGENCIA"; remotePaused = true; return;
   }

   if(!IsTradingTime() || TimeCurrent() < coolingEndTime) {
      botStatus = (TimeCurrent() < coolingEndTime) ? "ENFRIANDO" : "FUERA HORA";
      ProtectAll(); DeletePendings(); ActualizarHUD(); return;
   }
...
void SyncWeb() {
   ...
   string postD="{\"purchaseId\":\""+PurchaseID+"\",\"account\":\""+IntegerToString(AccountInfoInteger(ACCOUNT_LOGIN))+"\",\"version\":\"7.00\",\"status\":\""+botStatus+"\"}";
...
void ProtectAll() {
   ...
   for(int i=PositionsTotal()-1; i>=0; i--) {
      if(posInfo.SelectByIndex(i) && posInfo.Magic()==activeMagic) {
         ...
         // TP Individual (v5.55 style)
         double target = (posInfo.Comment()=="RESCATE") ? Target_Rescue_USD : Target_Harvest_USD;
         if(pUSD >= target) { trade.PositionClose(posInfo.Ticket()); continue; }
         ...
         if(ActivarBE && pUSD >= BE_Trigger_USD) {
            ...
            if(posInfo.PositionType()==POSITION_TYPE_BUY) {
               ...
               if(ActivarTrailing) { 
                  ...
                  if(ts>sl+TrailingStep*_Point) trade.PositionModify(posInfo.Ticket(),NormalizeDouble(ts,_Digits),0);
               }
            } else {
               double be=posInfo.PriceOpen()-2*_Point; 
               if(sl>be || sl==0) trade.PositionModify(posInfo.Ticket(),NormalizeDouble(be,_Digits),0);
               if(ActivarTrailing) { 
                  ...
                  if(ts<sl-TrailingStep*_Point || sl==0) trade.PositionModify(posInfo.Ticket(),NormalizeDouble(ts,_Digits),0);
               }
            }
         }
         
         // Smart Rescue Activation
         if(pUSD <= -GatilloRescate_USD && posInfo.Comment()!="RESCATE") {
            LanzarRescate(posInfo.Ticket());
         }
      }
   }
...
void LanzarRescate(ulong ticket) {
   if(!posInfo.SelectByTicket(ticket)) return;
   for(int i=0; i<PositionsTotal(); i++) if(posInfo.SelectByIndex(i) && posInfo.Magic()==activeMagic && posInfo.Comment()=="RESCATE") return;
   double lot = NormalizeDouble(posInfo.Volume()*1.5, 2);
   if(posInfo.PositionType()==POSITION_TYPE_BUY) trade.Sell(lot, _Symbol, SymbolInfoDouble(_Symbol,SYMBOL_BID), 0, 0, "RESCATE");
   else trade.Buy(lot, _Symbol, SymbolInfoDouble(_Symbol,SYMBOL_ASK), 0, 0, "RESCATE");
}
...
void ManageGates() {
   ...
   for(int i=OrdersTotal()-1; i>=0; i--) {
      ...
      if(OrderSelect(t) && OrderGetInteger(ORDER_MAGIC)==activeMagic) {
         ...
         if(OrderGetInteger(ORDER_TYPE)==ORDER_TYPE_BUY_STOP) {
            ...
            if(MathAbs(OrderGetDouble(ORDER_PRICE_OPEN)-p)>50*_Point) trade.OrderModify(t,NormalizeDouble(p,_Digits),0,0,ORDER_TIME_GTC,0,0);
         } else {
   ...
   if(!hasBuyStop && currentDir!=DIR_SOLO_VENTAS) trade.BuyStop(LoteManual,SymbolInfoDouble(_Symbol,SYMBOL_ASK)+dPoints,_Symbol,0,0,0,0,"G_BUY");
   if(!hasSellStop && currentDir!=DIR_SOLO_COMPRAS) trade.SellStop(LoteManual,SymbolInfoDouble(_Symbol,SYMBOL_BID)-dPoints,_Symbol,0,0,0,0,"G_SELL");
}
...
int OrdersTotalBots() { int c=0; for(int i=0; i<OrdersTotal(); i++) if(OrderSelect(OrderGetTicket(i)) && OrderGetInteger(ORDER_MAGIC)==activeMagic) c++; return c; }
...
void CrearHUD() {
   ...
   CrLabel("ttl",x+15,y+10,"EVOLUTION v7.0 HYBRID",clrWhite,10,"Arial Bold");
