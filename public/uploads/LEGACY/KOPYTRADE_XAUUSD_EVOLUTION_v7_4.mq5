//+------------------------------------------------------------------+
//|     KOPYTRADE_XAUUSD_EVOLUTION_v7_4 - EMERGENCY MASTER           |
//|   DIRECT ENTRY + -$3.0 AUTO-HEDGE (MARKET IF LATE) + UNIVERSAL BE|
...
#property strict
#property description "EMERGENCY MASTER | Market Hedge if Late | Universal BE"
...
void SyncWeb() {
   ...
   string postD="{\"purchaseId\":\""+PurchaseID+"\",\"account\":\""+IntegerToString(AccountInfoInteger(ACCOUNT_LOGIN))+"\",\"version\":\"7.40\",\"status\":\""+botStatus+"\"}";
      ...
      if(posInfo.SelectByIndex(i) && posInfo.Magic()==activeMagic) {
         ...
         if(ActivarBE && pUSD >= BE_Trigger_USD) {
            ...
            if(posInfo.PositionType()==POSITION_TYPE_BUY) {
               double be=posInfo.PriceOpen()+5*_Point; 
...
void ManageStrike() {
   ...
   for(int i=0; i<PositionsTotal(); i++) {
      if(posInfo.SelectByIndex(i) && posInfo.Magic()==activeMagic) {
         ...
         pUSD = GetProfitUSD(posInfo.Profit()+posInfo.Swap()+posInfo.Commission());
         break;
      }
   }
   
   // Check if we have another position in opposite direction
   for(int i=0; i<PositionsTotal(); i++) {
      if(posInfo.SelectByIndex(i) && posInfo.Magic()==activeMagic && posInfo.PositionType() != mainType) { hasOpposite=true; break; }
   }
   
   if(!hasOpposite && pUSD <= -GatilloHedge_USD) {
      // 1. Try to find a pending stop order
      ...
      if(!hasPending) {
         // EMERGENCY: If we are here, we are losing but have no hedge.
         // If price is ALREADY past the entryP +/- 3.0, entering a STOP is impossible.
         // We must ENTER MARKET to lock the hedge.
         ...
         if(pastHedge) {
            // Price already jumped the fence! Market Hedge NOW.
            if(mainType==POSITION_TYPE_BUY) trade.Sell(LoteManual*1.5,_Symbol,bid,0,0,"EMERGENCY_HEDGE");
            else trade.Buy(LoteManual*1.5,_Symbol,ask,0,0,"EMERGENCY_HEDGE");
         } else {
            // Price is still safe, place the static fence.
            if(mainType==POSITION_TYPE_BUY) trade.SellStop(LoteManual*1.5,targetHedgeP,_Symbol,0,0,0,0,"GUARD");
            else trade.BuyStop(LoteManual*1.5,targetHedgeP,_Symbol,0,0,0,0,"GUARD");
         }
      }
   }
}
...
void CrearHUD() {
   ...
   CrLabel("ttl",x+15,y+10,"EVO v7.4 EMERGENCY MASTER",clrWhite,10,"Arial Bold");
