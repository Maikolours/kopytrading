//+------------------------------------------------------------------+
//|     KOPYTRADE_BTCUSD_EVOLUTION_v7_3 - DIRECT STRIKE MASTER       |
...
#property strict
#property description "BTC DIRECT STRIKE | Analyze & Enter | -$3.0 Static Guard | Universal BE"
...
input int      DistanciaGuard      = 3000;         // Más espacio para BTC (30.00$)
      ...
      if(posInfo.SelectByIndex(i) && posInfo.Magic()==activeMagic) {
         ...
         if(ActivarBE && pUSD >= BE_Trigger_USD) {
            ...
            if(posInfo.PositionType()==POSITION_TYPE_BUY) {
               double be=posInfo.PriceOpen()+SymbolInfoDouble(_Symbol,SYMBOL_POINT); 
...
void ManageStrike() {
   ...
   for(int i=0; i<PositionsTotal(); i++) {
      if(posInfo.SelectByIndex(i) && posInfo.Magic()==activeMagic) {
         ...
         double pUSD = GetProfitUSD(posInfo.Profit()+posInfo.Swap()+posInfo.Commission());
         if(pUSD <= -GatilloHedge_USD) {
            // Check for existing guard order
            for(int j=0; j<OrdersTotal(); j++) if(OrderSelect(OrderGetTicket(j)) && OrderGetInteger(ORDER_MAGIC)==activeMagic) { hasGuard=true; break; }
            if(!hasGuard) {
               if(mainType == POSITION_TYPE_BUY) trade.SellStop(LoteManual*1.5, entryP - DistanciaGuard*_Point, _Symbol, 0, 0, 0, 0, "GUARD");
               else trade.BuyStop(LoteManual*1.5, entryP + DistanciaGuard*_Point, _Symbol, 0, 0, 0, 0, "GUARD");
            }
         }
         break;
      }
...
void CrearHUD() {
   ...
   CrLabel("ttl",x+15,y+10,"BTC EVO v7.3 MASTER",clrWhite,10,"Arial Bold");
