//+------------------------------------------------------------------+
//|     KOPYTRADE_XAUUSD_EVOLUTION_v7_1 - TRAILING ENTRY MASTER      |
//|   PULLBACK HUNTING + v6.10 SAFETY + MOBILE HUD + AUTO-VISUALS    |
...
#property strict
#property description "TRAILING ENTRY MASTER | Pullback Hunting | Smart Rescue"
...
input int      MagicNumber        = 700710;       
input int      MaxPosiciones      = 3;            
input double   LoteManual         = 0.02;         
input double   MaxDrawdown_USD    = 50.0;         
input double   Max_DD_Individual_USD = 25.0;      

sinput string separator2 = "=========================="; // === TÁCTICA TRAILING ENTRY ===
input int      DistanciaGates      = 150;         // Distancia entrada (v5.55 feel)
input int      MaxPersecucion      = 500;         // Puntos máximos de retroceso (Anclaje)
input double   GatilloRescate_USD = 5.0;          
input double   Target_Harvest_USD = 1.0;          
input double   Target_Rescue_USD  = 2.0;          
...
void SyncWeb() {
   ...
   string postD="{\"purchaseId\":\""+PurchaseID+"\",\"account\":\""+IntegerToString(AccountInfoInteger(ACCOUNT_LOGIN))+"\",\"version\":\"7.10\",\"status\":\""+botStatus+"\"}";
...
void ProtectAll() {
   ...
   for(int i=PositionsTotal()-1; i>=0; i--) {
      if(posInfo.SelectByIndex(i) && posInfo.Magic()==activeMagic) {
         ...
         if(pUSD <= -GatilloRescate_USD && posInfo.Comment()!="RESCATE") LanzarRescate(posInfo.Ticket());
      }
   }
...
void ManageGates() {
   ...
   for(int i=OrdersTotal()-1; i>=0; i--) {
      ...
      if(OrderSelect(t) && OrderGetInteger(ORDER_MAGIC)==activeMagic) {
         ...
         if(type==ORDER_TYPE_BUY_STOP) {
            hasBuyStop=true;
            double targetP = SymbolInfoDouble(_Symbol,SYMBOL_ASK)+dPoints;
            // TRAILING ENTRY: Solo baja si no ha bajado ya el máximo permitido
            ...
            hasSellStop=true;
            double targetP = SymbolInfoDouble(_Symbol,SYMBOL_BID)-dPoints;
            // TRAILING ENTRY: Solo sube si no ha subido ya el máximo permitido
   ...
   if(!hasBuyStop && trendUp && (currentDir==DIR_AMBAS || currentDir==DIR_SOLO_COMPRAS)) 
      trade.BuyStop(LoteManual,SymbolInfoDouble(_Symbol,SYMBOL_ASK)+dPoints,_Symbol,0,0,0,0,"G_BUY");
   if(!hasSellStop && !trendUp && (currentDir==DIR_AMBAS || currentDir==DIR_SOLO_VENTAS)) 
...
void CrearHUD() {
   ...
   CrLabel("ttl",x+15,y+10,"EVO v7.1 TRAILING ENTRY",clrWhite,10,"Arial Bold");
