//+------------------------------------------------------------------+
//|    KOPYTRADE_XAUUSD_EVOLUTION_v8_3_0 - TITAN SHIELD              |
...
#property strict
#property description "TITAN SHIELD v8.3.1 | Web Sync & Remote Management"
...
input int      MagicNumber       = 800801;
input double   LotSize           = 0.03;      
input int      MaxOpenTrades     = 2;         
input double   MaxDrawdownPct    = 10.0;      
input int      MinImpulsePts     = 5000;      

sinput string separator2 = "=========================="; // === FILTROS v8.2.1 ===
input int      MaxReEntries      = 2;         
input int      MaxLateEntryPts   = 100;       
input int      MinM5Vol          = 50;        
input int      MaxM5Vol          = 1500;      
...
string         botStatus = "STARTING ARCHITECT...", PNL = "AEVO_v821_GOLD";
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam) {
   if(id == CHARTEVENT_OBJECT_CLICK) {
      if(sparam == PNL+"btn_close") {
         if(MessageBox("¿CERRAR TODAS LAS POSICIONES DEL BOT?","TITAN EMERGENCY",MB_YESNO|MB_ICONWARNING) == IDYES) {
            CloseAllBotPositions();
            botStatus = "!!! EMERGENCY CLOSED !!!";
            ActualizarHUD();
         }
      }
...
void OnDeinit(const int r) { ObjectsDeleteAll(0, PNL); ObjectsDeleteAll(0, "F_LVL_"); }
      ...
      if(!broken) {
         if(PositionsTotalBots() < MaxOpenTrades) {
            if(MathAbs(f_0 - f_100) > MinImpulsePts * _Point) {
               ScanForEntries();
               if(PositionsTotalBots()==0) botStatus = "🎯 IRON ARCHITECT HUNTING...";
            } else botStatus = "L4TERAL - WAITING";
         } else botStatus = "MAX EXPOSURE HIT";
      } else botStatus = "STRUCTURE BROKEN (78.6)";
   } else botStatus = "Zzz (FUERA DE HORA)";

   if(f_dir == 0) botStatus = "⚠️ CARGANDO HISTORIAL H1...";
...
void TryConfirm(ENUM_FIBO_LEVEL lvl, ENUM_POSITION_TYPE type) {
   ...
   if(range < MinM5Vol*_Point || range > MaxM5Vol*_Point) { botStatus = "VOL. FILTER SKIP"; return; }
   
   double target = (lvl==LVL_38)?f_38:(lvl==LVL_50?f_50:f_61);
   double price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   if(MathAbs(price - target)/_Point > MaxLateEntryPts) { botStatus = "LATE ENTRY SKIP"; return; }
   ...
   if(wickOk && colorOk) {
      ...
      stats[idx].entries++;
      botStatus = "L"+IntegerToString(lvl)+" ESCUDO ACTIVO";
   }
}
...
void DrawFiboMap() {
   ...
   // Smooth Redraw: No borramos todo, actualizamos posiciones
   string dir = (f_dir == 1) ? "BUY" : "SELL";
   
   CrZone("GZone", f_50, f_61, C'40,30,10');
   CrFLine("38", f_38, C'150,150,180', "38.2% - " + dir, 1);
   CrFLine("50", f_50, C'200,200,250', "50.0% - " + dir, 2);
   CrFLine("61", f_61, C'255,180,0',   "61.8% - ORO", 3);
   CrFLine("78", f_78, C'255,50,50',   "!!! 78.6% - ROTURA !!!", 2);
   CrFLine("00", f_0,  C'0,255,150',   "OBJETIVO - 0.0%", 1);
   CrFLine("100", f_100, C'255,80,80', "STOP - 100.0%", 1);
}

void CrFLine(string n, double p, color c, string t, int w=1) {
   string name = "F_LVL_"+n, lname = "F_LBL_"+n;
   
   if(ObjectFind(0, name) < 0) {
      ObjectCreate(0, name, OBJ_HLINE, 0, 0, p);
      ObjectSetInteger(0, name, OBJPROP_ZORDER, 0); // Capa fondo
   ...
   ObjectSetString(0, lname, OBJPROP_TEXT, "  ◀ " + t);
...
void CrearHUD() {
   ...
   int x=15, y=30, w=(isMin?220:300), h=(isMin?40:330);
   
   // Fondo y Título
   CrRect("bg",x,y,w,h,C'10,10,25',C'80,120,200',2);
   CrLabel("ttl",x+15,y+10,"🏗️ EVO v8.3.0 TITAN SHIELD",C'0,255,180',isMin?8:11,isMin?"Arial":"Impact");
   CrButton("btn_min",x+w-30,y+10,20,20,isMin?"+":"-",C'50,50,80',clrWhite);

   if(isMin) return;

   // Status Line
   CrRect("st_bg",x+10,y+40,w-20,25,C'30,30,50',C'50,60,100',1);
   CrLabel("stV",x+20,y+45,botStatus,clrWhite,9,"Arial Black");
   
   // Datos de Mercado (Grid)
   int gy = y+75;
   CrLabel("l_trnd",x+15,gy,   "TENDENCIA:",C'0,200,255',8);     CrLabel("trend",x+110,gy,"BUSCANDO...",clrWhite,8,"Arial Black");
   CrLabel("h_l",x+15,gy+20,   "MÁXIMO SWING:",C'150,150,180',8);  CrLabel("hi",x+110,gy+20,"---",clrWhite,8,"Arial Bold");
   CrLabel("l_l",x+15,gy+38,"MÍNIMO SWING: ",C'150,150,180',8);  CrLabel("lo",x+110,gy+38,"---",clrWhite,8,"Arial Bold");
   
   // Parámetros Actuales (Exposure)
   int py = gy+65;
   CrRect("sep1",x+15,py-5,w-30,1,C'60,60,80',C'60,60,80',1);
   ...
   CrLabel("l_mx",x+15,py+54, "MAX OP:",C'255,150,100',8);   CrLabel("v_mx",x+90,py+54,IntegerToString(MaxOpenTrades),clrWhite,8,"Arial Bold");

   // BOTONES
   int by = y+h-50;
   CrButton("btn_close",x+15,by,w-30,35,"🛑 CIERRE DE EMERGENCIA TOTAL",C'120,30,30',clrWhite);
}
...
void CrButton(string n, int x, int y, int w, int h, string t, color bg, color tc) {
   ...
   ObjectSetInteger(0, name, OBJPROP_ZORDER, 100); // Prioridad alta
...
void ActualizarHUD() {
   ...
   ObjectSetString(0,PNL+"hi",OBJPROP_TEXT,DoubleToString(f_dir==1?f_0:f_100,_Digits));
   ObjectSetString(0,PNL+"lo",OBJPROP_TEXT,DoubleToString(f_dir==1?f_100:f_0,_Digits));
   for(int i=1; i<=3; i++) if(stats[i].locked) {
      ObjectSetInteger(0,PNL+"st_bg",OBJPROP_BGCOLOR,C'80,30,30');
      ObjectSetString(0,PNL+"stV",OBJPROP_TEXT,"MODO SEGURO: L"+IntegerToString(i)+" BLOQUEADO");
   }
}
...
void CrRect(string n, int x, int y, int w, int h, color bg, color bd, int bdw=1) {
   ...
   ObjectSetInteger(0, name, OBJPROP_ZORDER, 50); // Superior al gráfico
   ObjectSetInteger(0, name, OBJPROP_SELECTABLE, false);
}
...
void CrLabel(string n, int x, int y, string t, color c, int s, string f="Arial") {
   ...
   ObjectSetInteger(0, name, OBJPROP_ZORDER, 60); // Encima de los rectángulos
