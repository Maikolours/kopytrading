//+------------------------------------------------------------------+
//| EA Scalping Inteligente v5.3 – MT5 (RESCATE MATEMÁTICO 0.02)     |
//| Con cierre individual independiente del rescate                  |
//+------------------------------------------------------------------+
#property strict
...
input bool   ActivarRescate     = false;     // 🚑 Activar Operación de Rescate Automática
input double MultiplicadorLote  = 2.0;       // 📦 Multiplicador de lote (ej: 0.01 -> 0.02)
input double DistanciaRescate   = 4.5;       // 📏 Distancia en Dólares ($) para activar el rescate
...
input double SL_Mult_Gold       = 1.2;       // Stop Loss ORO (Multiplicador ATR)
...
void ManageRescate()
{
   ...
   // 2. Si hay 1 BUY y va perdiendo la distancia, rescate en SELL
   ...
   // 3. Si hay 1 SELL y va perdiendo la distancia, rescate en BUY
...
void ManageCortafuegos()
{
   ...
   // AHORA LAS CIERRA INDIVIDUALMENTE CON SU PROPIO UMBRAL DE 6$ 
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ulong ticket = PositionGetTicket(i);
      if(!PositionSelectByTicket(ticket)) continue;
      ...
      // Si la operación concreta pierde o gana $6, se cierra ella sola
...
void DrawPanel(double dailyProfit)
{
   ...
   string text = "╔══════════════════════════════════════════════╗\n";
   text += "║  EA Scalping v5.3 [Ajustes de Cierre Originales] \n";
   text += "╠══════════════════════════════════════════════╣\n";
...
void OnTick()
{
   ...
   // ENTRADA NORMAL SIN IMPORTAR SI HAY POSICIONES O NO (Como la v5.1)
