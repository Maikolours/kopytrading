//+------------------------------------------------------------------+
//| EA Scalping Inteligente v4.3 – MT5 (REPARADO Y REVISADO)         |
//+------------------------------------------------------------------+
#property strict
...
//+------------------------------------------------------------------+
//| Gestionar Break Even y Trailing (REPARADO Y BLINDADO)            |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Dibujar panel (ARREGLADO TEXTO Y RECUADRO)                       |
//+------------------------------------------------------------------+
void DrawPanel()
{
   if(!ShowPanel) return;
   string bgName = panelName + "bg";
   string txtName = panelName + "txt";
   ...
   // [ESTA ERA LA LÍNEA QUE BORRÉ POR ERROR Y QUE DIBUJA EL TEXTO]
   if(ObjectFind(0, txtName) < 0) {
      ObjectCreate(0, txtName, OBJ_LABEL, 0, 0, 0);
      ObjectSetInteger(0, txtName, OBJPROP_XDISTANCE, 15);
      ObjectSetInteger(0, txtName, OBJPROP_YDISTANCE, 35);
      ObjectSetInteger(0, txtName, OBJPROP_FONTSIZE, 9);
      ObjectSetInteger(0, txtName, OBJPROP_COLOR, clrLime);
   }

   string text = "EA Scalping Inteligente v4.3 [Limpio]\n";
   ...
   text += "BE: " + (EnableBE ? "ON" : "OFF") + " | Trail: " + (EnableTrailing ? "ON" : "OFF");
...
void OnTick()
{
   ...
   // ENTRADA BUY TÚ ORIGINAL (HASTA Max trades per side)
   ...
   // ENTRADA SELL TÚ ORIGINAL (HASTA Max trades per side)
