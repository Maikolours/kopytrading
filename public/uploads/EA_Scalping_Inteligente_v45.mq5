//+------------------------------------------------------------------+
//| EA Scalping Inteligente v4.5 – MT5 (CON CORTAFUEGOS DE PÉRDIDA)  |
//+------------------------------------------------------------------+
#property strict
...
input int    MaxVelasContra      = 4;        // 🕯️ Cortar operación si han pasado X velas y sigue en rojo
...
input double BE_Trigger_USD      = 3.0;      // Ganancia en USD para activar BE
input double BE_Cushion_USD      = 1.0;      // Ganancia asegurada en USD después de BE

// ================= TRAILING STOP =================
input bool   EnableTrailing      = true;     // Activar Trailing Stop
input double Trail_Trigger_USD   = 3.0;      // Ganancia en USD para activar trailing
...
void ManageCortafuegos()
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      // Regla 1: Cerrar instantáneamente si pasa de X dólares de pérdida
      if(profit <= -MaxPerdida_USD) 
      {
         trade.PositionClose(ticket);
         Print("🛡️ CORTAFUEGOS 1: Operación cerrada por llegar a pérdida máx: $", DoubleToString(profit, 2));
         continue;
      }
      
      // Regla 2: Cerrar si han pasado X velas y sigue en negativo (Estancamiento)
      ...
      {
         trade.PositionClose(ticket);
         Print("⏱️ CORTAFUEGOS 2: Operación cerrada por estancamiento de ", barsOpen, " velas en negativo.");
         continue;
      }
   }
}
...
void DrawPanel()
{
   ...
   string text = "╔══════════════════════════════════════════════╗\n";
   text += "║  EA Scalping v4.5 [Con Cortafuegos 🛡️] \n";
   text += "╠══════════════════════════════════════════════╣\n";
   ...
   text += "║\n";
   text += "║ Cortafuegos Máx USD: $" + DoubleToString(MaxPerdida_USD, 2) + "\n";
   text += "║ Cortafuegos Tiempo: " + IntegerToString(MaxVelasContra) + " Velas rojas\n";
   text += "╚══════════════════════════════════════════════╝\n";
   Comment(text);
}
...
void OnTick()
{
   ManageBETrailing();
   ManageTimeExit(); 
   ManageCortafuegos(); // <-- Nueva línea de defensa añadida
