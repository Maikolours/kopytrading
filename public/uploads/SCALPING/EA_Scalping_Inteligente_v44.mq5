//+------------------------------------------------------------------+
//| EA Scalping Inteligente v4.4 – MT5 (PANEL ARREGLADO)             |
//+------------------------------------------------------------------+
#property strict
...
void OnDeinit(const int reason)
{
   ...
   Comment(""); // Limpiar panel de MQL5 native
}
...
void DrawPanel()
{
   ...
   string text = "╔══════════════════════════════════════════════╗\n";
   text += "║  EA Scalping Inteligente v4.4 [Clean]  \n";
   text += "╠══════════════════════════════════════════════╣\n";
   text += "║ Balance: " + DoubleToString(AccountInfoDouble(ACCOUNT_BALANCE), 2) + " USD\n";
   text += "║ Posiciones Oro: " + IntegerToString(PositionsTotal()) + "\n";
   text += "║ Compras: " + IntegerToString(CountTrades(POSITION_TYPE_BUY)) + " de " + IntegerToString(MaxTradesPerSide) + "\n";
   text += "║ Ventas: " + IntegerToString(CountTrades(POSITION_TYPE_SELL)) + " de " + IntegerToString(MaxTradesPerSide) + "\n";
   text += "║ Protección BE: " + (EnableBE ? "ACTIVO" : "APAGADO") + " | Trail: " + (EnableTrailing ? "ACTIVO" : "APAGADO") + "\n";
   text += "╚══════════════════════════════════════════════╝\n";
...
void OnTick()
{
   ...
   // Dibujamos el panel en cada tick para que esté super actualizado
   DrawPanel();
