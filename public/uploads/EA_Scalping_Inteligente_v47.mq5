//+------------------------------------------------------------------+
//| EA Scalping Inteligente v4.7 – MT5 (AJUSTES FINALES)             |
//+------------------------------------------------------------------+
#property strict
...
input double MaxPerdida_USD      = 6.0;      // 💸 Cortar operación inmediatamente si pierde X dólares (AJUSTADO A 6)
input int    MaxVelasContra      = 2;        // 🕯️ Cortar operación si han pasado X velas y sigue en rojo
...
input double Manual_TP_USD       = 6.0;      // 🎯 Take Profit en USD (0 = Desactivado, usará el ATR)
...
void ManageCortafuegos()
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      // Añadimos el nuevo Take Profit opcional en dólares al final del cortafuegos
      if(Manual_TP_USD > 0 && profit >= Manual_TP_USD)
      {
         trade.PositionClose(ticket);
         continue;
      }
...
void DrawPanel()
{
   ...
   string text = "╔══════════════════════════════════════════════╗\n";
   text += "║  EA Scalping v4.7 [Con TP Manual] \n";
   text += "╠══════════════════════════════════════════════╣\n";
   ...
   if(Manual_TP_USD > 0) text += "║ Take Profit Opcional: $" + DoubleToString(Manual_TP_USD, 2) + "\n";
   text += "╚══════════════════════════════════════════════╝\n";
   Comment(text);
}
...
void OnTick()
{
   ...
   if(CheckMomentum(true) && CountTrades(POSITION_TYPE_BUY) < MaxTradesPerSide)
   {
      ...
      sl = price - atr * slMult;
      if(Manual_TP_USD > 0) tp = 0; // Si hay TP manual en $, no ponemos línea verde de ATR
      else tp = price + atr * tpMult; // Tp clásico
   ...
   if(CheckMomentum(false) && CountTrades(POSITION_TYPE_SELL) < MaxTradesPerSide)
   {
      ...
      sl = price + atr * slMult;
      if(Manual_TP_USD > 0) tp = 0; 
      else tp = price - atr * tpMult; 
