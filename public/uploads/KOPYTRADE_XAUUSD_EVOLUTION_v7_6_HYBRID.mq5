//+------------------------------------------------------------------+
//|     KOPYTRADE_XAUUSD_EVOLUTION_v7_6 - HYBRID RECOVERY            |
...
#property strict
#property description "HYBRID EVOLUTION | Momentum + Mean Reversion Recovery"
...
//--- PARÁMETROS ---
enum ENUM_ACCOUNT_TYPE { ACCOUNT_STANDARD, ACCOUNT_CENT, ACCOUNT_DEMO };
input ENUM_ACCOUNT_TYPE AccountType = ACCOUNT_CENT; // Tipo de Cuenta (Cent, Normal, Demo)
...
input double   GatilloHedge_USD    = 0.50;         // Bajado a 0.50 para Cent
input double   Target_Rescate      = 0.20;          
input double   Net_Cycle_USD       = 2.0;          // Bajado a 2.00 para Cent
...
input bool     EnableLimitRecovery = true;         // Activar compras/ventas en retroceso
input int      LimitDistPoints     = 500;          // Distancia para la orden LIMIT
...
string         botStatus = "LISTO", commentTag = "EVO_HYBRID_76";
...
int OnInit() {
   ...
   if(AccountType == ACCOUNT_CENT || (tv < 0.1 && AccountInfoInteger(ACCOUNT_TRADE_MODE) == ACCOUNT_TRADE_MODE_REAL)) profitFactor = 0.01;
   else profitFactor = 1.0;
   
   // Sugerencia de lote automático si está en default 0.01 y es cuenta CENT
   if(AccountType == ACCOUNT_CENT && LoteManual == 0.01) {
      dynLoteManual = 0.10; // Lote recomendado para $250 Cent
      dynLoteRescate = 0.20;
   } else {
...
void OnTick() {
   ...
   if(TimeCurrent() < coolingEndTime) { botStatus = "ENFRIANDO"; ProtectAll(); DeletePendings(); ActualizarHUD(); return; }
...
void ManageStrike() {
   ...
   if(posTotal == 0) {
      ...
      double distUSD = (distPoints / 100) * (dynLoteManual * 100) * profitFactor; 
      if(distUSD > MaxDistEMA_USD) { botStatus = "PRECIO MUY LEJOS (ESPERANDO)"; return; }

      if(currentMode == MODE_COSECHA) {
         if(rsi[0] < RSI_Oversold) trade.Buy(dynLoteManual,_Symbol,ask,0,0,commentTag);
         else if(rsi[0] > RSI_Overbought) trade.Sell(dynLoteManual,_Symbol,bid,0,0,commentTag);
      } else {
         if(rsi[0] > 50) trade.Buy(dynLoteManual,_Symbol,ask,0,0,commentTag);
         else trade.Sell(dynLoteManual,_Symbol,bid,0,0,commentTag);
      }
      return;
   }
   ...
   if(net <= -dynGatilloHedge) {
      ...
      if(!hasPending) {
         // Buscamos la posición más lejana para basar el rescate
         long type = -1;
         for(int i=0; i<PositionsTotal(); i++) if(posInfo.SelectByIndex(i) && posInfo.Magic()==activeMagic) { type = posInfo.PositionType(); break; }
         
         if(type == POSITION_TYPE_BUY) {
            // Precio cae -> Necesitamos compras más abajo (LIMIT) o ventas para cubrir (STOP)
            if(EnableLimitRecovery) trade.BuyLimit(dynLoteRescate, bid - dynLimitDist*_Point, _Symbol, 0,0,0,0, commentTag+"_LIM");
            trade.SellStop(dynLoteRescate, bid - 300*_Point, _Symbol, 0,0,0,0, commentTag+"_STP");
         } else {
            // Precio sube
            if(EnableLimitRecovery) trade.SellLimit(dynLoteRescate, ask + dynLimitDist*_Point, _Symbol, 0,0,0,0, commentTag+"_LIM");
            trade.BuyStop(dynLoteRescate, ask + 300*_Point, _Symbol, 0,0,0,0, commentTag+"_STP");
         }
      }
   }
}
...
void DrawGhostLines() {
   ...
   for(int i=0; i<PositionsTotal(); i++) {
      if(posInfo.SelectByIndex(i) && posInfo.Magic()==activeMagic) { 
         ...
         break; // Usamos el precio de la primera del bloque
      }
   }
   ...
   // Distancia en puntos para alcanzar el gatillo:
   // Perdida = Puntos * _Point * (TickVal/TickSize) * Lots * ProfitFactor
   // Puntos = Perdida / (_Point * (TickVal/TickSize) * Lots * ProfitFactor)
   ...
   double stpP = (type==POSITION_TYPE_BUY) ? triggerP - 300*_Point : triggerP + 300*_Point;
   ...
   CrHLine("_GHOST_STP", NormalizeDouble(stpP, _Digits), C'255,50,50', STYLE_DASH, 1, "Proyección STOP a 300 pts");
}
...
void CrearHUD() {
   ...
   CrLabel("ttl",x+15,y+10,"EVO v7.6 HYBRID",clrWhite,10,"Arial Bold");
