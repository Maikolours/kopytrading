//+------------------------------------------------------------------+
//|            ELITE GOLD MAIKO SNIPER v11.30 | HEDGED S/R          |
...
input int ExpertMagic = 111223; // Magic Number (Debe ser unico para cada bot en la misma cuenta)
input bool EsCuentaCent = false; // Es Cuenta Cent? (Multiplica TP por 100)
...
input double ProfitNetoFlush = 5.0;
input double ProfitCosechaIndividual = 1.0;
...
// --- HUD ---
input string HUD_Branding = "MAIKO v11.30 | HEDGED S/R";
...
input string TradeComment = "MAIKO_HEDGED_SR";
...
double CalcularMetaEscapeTPBasket(int type) {
    ...
    double targetUSD = (targetActual - sumCommSwap) / multCent;
...
void GestionarCosechaSniper() { 
    ...
    for(int i=ArraySize(posBuy)-1; i>=0; i--) {
        if((posBuy[i].p + posBuy[i].c + posBuy[i].s) >= (ProfitCosechaIndividual * multCent)) {
            trade.PositionClose(posBuy[i].ticket);
        }
    }
    for(int i=ArraySize(posSell)-1; i>=0; i--) {
        if((posSell[i].p + posSell[i].c + posSell[i].s) >= (ProfitCosechaIndividual * multCent)) {
...
void OnTick() {
    ...
    // Verificar cierres globales/cesta
    if(ArraySize(posBuy) > 0) {
        double targetBuy = (ArraySize(posBuy) >= LimitePosicionesSOS) ? (ProfitBreakEven * multCent) : (ProfitNetoFlush * multCent);
    ...
    if(ArraySize(posSell) > 0) {
        double targetSell = (ArraySize(posSell) >= LimitePosicionesSOS) ? (ProfitBreakEven * multCent) : (ProfitNetoFlush * multCent);
    ...
    if(ProteccionBeneficioDiario > 0.0 && ganadoHoy > (ProteccionBeneficioDiario * multCent)) {
        if((ganadoHoy + flotante) <= (ProteccionBeneficioDiario * multCent) && (ArraySize(posBuy) > 0 || ArraySize(posSell) > 0)) {
