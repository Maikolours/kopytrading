...
input double BeneficioActivacionTrailing = 2.0; // Beneficio ($) para activar Trailing
input double DistanciaTrailing = 1.0;      // Distancia de arrastre ($)
// --- NUEVOS FILTROS ---
...
input int LimitePosicionesSOS = 2; 
...
input double ProfitEmergenciaCents = 75.0; // TP de emergencia global (cents)

input double ProfitNetoCents = 75.0;   
...
string TradeComment = "HYBRID_GOLD";
...
// --- MODO LATERAL (Desconexi�n de H1/H4 si no hay tendencia) ---
    ...
    if(CheckH4 && !enModoLateral) { condBuy = condBuy && (bid > emaH4[0]); condSell = condSell && (bid < emaH4[0]); }
    if(CheckH1 && !enModoLateral) { condBuy = condBuy && (bid > emaH1[0]); condSell = condSell && (bid < emaH1[0]); }
...
void CrearInterfazMaster() { 
    ...
    CrearLabel("MAIKO_T", x+10, y+10, "MAIKO HYBRID | GOLD PRO v13.92", ColorMain, 11, 10001); 
