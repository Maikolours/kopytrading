//+------------------------------------------------------------------+
//|                MAIKO SNIPER PRO | EDITION GOLD V11              |
//|      "BUTTON FIX & SAFE RECOVERY" | VERSION 11.99 FINAL         |
//+------------------------------------------------------------------+
...
input bool EsCuentaCent = false; // CUENTA DEMO / GOLD
...
// --- GESTION DE LOTAJE ---
input double LotajeInicial = 0.01; // Ajustado a 0.01 para Gold
input double MultiplicadorRefuerzo = 1.0; 
input double MaxLoteIndividual = 0.02; // Ajustado a 0.02 para Gold
...
// --- OBJETIVOS DE PROFIT ---
input double ProfitCosechaUSD = 2.0; // En cuenta Gold es en dólares
input double ProfitNetoUSD = 10.0; // En cuenta Gold es en dólares
input double DistanciaRefuerzoPips = 30.0; 

// --- PROTECCION DE EQUIDAD ---
input bool InpUsarProteccionEquidad = true; // Activar Stop Loss por Equidad (Drawdown)
input double InpMaxDrawdownPorcentaje = 20.0; // Drawdown maximo (%) para Stop Loss
...
input string TradeComment = "MAIKO_GOLD_V11";
...
void OnTick() {
    ...
    // LOGICA DE PROTECCION DE EQUIDAD
    ...
    if(UsarProteccionEquidad && equityPeak > 0) {
        ...
        if(ddPercent >= MaxDrawdownPorcentaje) {
            CerrarTodo();
            equityPeak = AccountInfoDouble(ACCOUNT_BALANCE); // Reiniciar la cima para empezar de cero
            txtVeredicto = "REINICIO TRAS STOP LOSS";
            txtVoz = "RECUPERACION ACTIVADA tras perdida.";
    ...
    double metaCesta = (ArraySize(pos) >= LimitePosicionesSOS) ? 0.25 : ProfitNetoUSD;
...
void GestionarCosechaSniper() { 
    for(int i=ArraySize(pos)-1; i>=0; i--) {
        ...
        double profitObjetivo = (pos[i].v / 0.01) * ProfitCosechaUSD;
...
void CrearInterfazMaster() { 
    ...
    CrearLabel("MAIKO_T", x+10, y+10, "MAIKO PRO | GOLD v11.99", ColorMain, 11, 10001); 
