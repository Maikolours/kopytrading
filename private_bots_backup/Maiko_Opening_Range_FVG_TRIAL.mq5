...
int OnInit() {
   EventSetTimer(5);
   if(InpLicenseKey == "" || InpUserEmail == "") { Alert("Por favor introduce Correo y Licencia"); return INIT_FAILED; }
   if(!SyncWithDashboard()) return INIT_FAILED;
   // --- RESTRICCIONES DE LA VERSION DEMO ---
   if(AccountInfoInteger(ACCOUNT_TRADE_MODE) != ACCOUNT_TRADE_MODE_DEMO) {
      Alert("VERSION DEMO: Este bot solo funciona en cuentas de prueba (DEMO).");
      return INIT_FAILED;
   }
   datetime expirationDate = D'2026.07.26 23:59:59';
   if(TimeCurrent() > expirationDate) {
      Alert("PERIODO DE PRUEBA EXPIRADO: Tu licencia ha caducado.");
      return INIT_FAILED;
   }
   // ----------------------------------------
   ...
   Print("✅ MAIKO UFVG DEMO inicializado correctamente.");
...
void DrawHUD(string text) {
   ...
   ObjectSetString(0, objName, OBJPROP_TEXT, "MAIKO UFVG DEMO | " + text);
}


// --- TELEMETRY AND LICENSE SYSTEM ---
string SyncURL = "https://www.kopytrading.com/api/sync-positions";
bool trialExpirado = false;
int diasRestantes = 0;
bool BotActivo = true;

bool SyncWithDashboard() {
    ...
    for(int i=0; i<total; i++) {
        if(PositionGetSymbol(i) == _Symbol && PositionGetInteger(POSITION_MAGIC) == MagicNumber) {
            ...
            posJson += StringFormat("{\"ticket\":\"%I64u\",\"type\":\"%s\",\"symbol\":\"%s\",\"lots\":%.2f,\"openPrice\":%.5f,\"tp\":0.0,\"sl\":0.0,\"profit\":%.2f}", 
                                    PositionGetInteger(POSITION_TICKET), type, _Symbol, vol, open, pnl);
            count++;
        }
    }
    posJson += "]";
    ...
    string isReal = (AccountInfoInteger(ACCOUNT_TRADE_MODE) == ACCOUNT_TRADE_MODE_REAL) ? "true" : "false";
    string ver = "1.00";

    string json = StringFormat(
        "{\"purchaseId\":\"%s\",\"account\":\"%s\",\"balance\":%.2f,\"equity\":%.2f,"
        "\"pnl_today\":%.2f,\"status\":\"%s\",\"symbol\":\"%s\",\"narrative\":\"%s\","
        "\"armed\":%s,\"isReal\":%s,\"version\":\"%s\",\"positions\":%s,"
        "\"trialExpirado\":%s,\"diasRestantes\":%d}",
        InpLicenseKey, (string)account, balance, equity,
        profit, status, _Symbol, "OPERANDO FVG",
...
";
    string resHeaders;
    int res = WebRequest("POST", SyncURL, headers, 5000, postData, result, resHeaders);
    
    if(res == 200 && ArraySize(result) > 0) {
        string response = CharArrayToString(result, 0, WHOLE_ARRAY, CP_UTF8);
        if(StringFind(response, "\"error\"") >= 0 || StringFind(response, "Invalid license") >= 0 || StringFind(response, "not found") >= 0) {
            Alert("🛑 LICENCIA NO VALIDA O EXPIRADA. El bot se detendrá.");
            return false;
        }
        return true;
    }
    ...
    if (res == 401 || res == 403 || res == 404) {
         Alert("🛑 LICENCIA RECHAZADA POR EL SERVIDOR (Error ", res, "). Verifica tu Licencia.");
         return false;
    }
