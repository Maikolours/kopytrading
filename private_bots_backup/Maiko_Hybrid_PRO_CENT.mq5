...
input double DistanciaCascadaPips = 5.0; 

input int MaxPosicionesCascada = 3; 
...
input bool UsarNoticiasAutomaticas = true; // Si es true, usa el calendario interno de MT5 (Noticias de alto impacto USD)

input string HoraProximaNoticia = "14:30"; // Solo se usa si UsarNoticiasAutomaticas = false
...
input double SegundosReAnalisis = 30.0;       // Segundos base de espera tras cerrar cesta
...
input bool UsarFiltroADX = true; // Activar Filtro ADX (H1 > 25)

input int ADX_MinLevel = 25; // Nivel Mínimo ADX para entrar (Tendencia)
...
// --- MODO DUAL (MOMENTUM & HEDGING) ---
input bool UsarModoMomentum = true;        // Activar subida a ola (Vela Elefante)
input double VelaElefantePips = 30.0;      // Tama�o m�nimo vela M15 para Momentum
input int MinADX_Lateral = 25;             // Si ADX < esto, es mercado lateral (Scalping)
input bool UsarTrailingCesta = true;       // Activar Trailing Stop para Cesta de Cobertura
input double BeneficioActivacionTrailing = 0.50; // Beneficio ($) para activar Trailing
input double DistanciaTrailing = 0.25;      // Distancia de arrastre ($)
// --- NUEVOS FILTROS ---
...
input double LotajeMinimo = 0.02;     // Lote para entradas 1 y 2 (mínimo, protege la cesta)
...
input int LimitePosicionesSOS = 0; 
...
input double ProfitScalpIndividual = 3.0; // Profit individual para posiciones SOS (cents)

input double ProfitScalpMinLote = 3.0;    // Profit individual para posiciones de 0.01 (cents)

input bool UsarEmergenciaAuto = true;    // Activa la salida de emergencia inteligente

input double ProfitEmergenciaCents = 0.50; // TP de emergencia global (cents)

input double ProfitNetoCents = 0.50;   

input double LimiteDiario = 10.0; // Lmite de beneficio diario ($/,~)
input bool IgnorarHistorialPrevioHoy = true; // Reiniciar marcador HOY a cero al arrancar

input double DistanciaRefuerzoPipsBase = 15.0; 
...
string TradeComment = "HYBRID_CENT";



// --- TRAILING STOP DE CESTA (DINAMICO) ---
double highestEquityTrailing = 0;
void GestionarTrailingCesta() {
    if (!UsarTrailingCesta) return;
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == ExpertMagic) {
            beneficioActual += PositionGetDouble(POSITION_PROFIT) + PositionGetDouble(POSITION_SWAP);
            totalPosiciones++;
        }
    }
    
    if (totalPosiciones == 0) {
        highestEquityTrailing = 0;
        return;
    }
    
    // Hit & Run: Si hay Hedge activo, cerramos rapido al tocar positivo
    bool hayHedge = false;
    for(int i=0; i<ArraySize(pos); i++) {
        if(StringFind(PositionGetString(POSITION_COMMENT), "_HEDGE") >= 0) { hayHedge = true; break; }
    }
    if(hayHedge && beneficioActual >= (CUENTA_REAL_CENT ? 50.0 : 0.50)) { // +0.50$ ganancia neta minima
        CerrarTodo();
        return;
    }
    
    if (beneficioActual > BeneficioActivacionTrailing) {
        if (beneficioActual > highestEquityTrailing) {
            highestEquityTrailing = beneficioActual; // Actualizar el pico m�ximo
        }
        
        // Si el beneficio retrocede desde el pico m�s all� de la distancia de trailing
        if (highestEquityTrailing - beneficioActual >= DistanciaTrailing) {
            txtProteccion = "TRAILING CERRANDO CESTA!";
            CerrarTodo();
            highestEquityTrailing = 0;
        }
    }
...
int OnInit() {
    eaStartTime = TimeCurrent();
      ...
      if((bool)MQLInfoInteger(MQL_TESTER)) BotActivo = true;
    ...
    // Gestión Inteligente de Indicadores Visuales (igual que BTC)
...
bool IsNewsBlocked() {
    ...
    if(UsarNoticiasAutomaticas) {
        ...
        if(CalendarValueHistory(values, start, end, "US")) {
...
void OnTick() {
    ...
    if(ArraySize(pos) > 1 && flotante >= (CUENTA_REAL_CENT ? 50.0 : 0.50)) { // Cierre Global de Cesta
        CerrarTodo();
        return;
    }
    ...
    bool condSell = (rsiActual < 50 && rsiActual > MinRSI_Venta);
        ...
        // --- MODO MOMENTUM (VELA ELEFANTE) ---
    bool esVelaElefanteAlcista = false;
    bool esVelaElefanteBajista = false;
    
    if(UsarModoMomentum && BotActivo) {
        // Mirar M15 (estructural)
        double openM15 = iOpen(_Symbol, PERIOD_M15, 0);
        double closeM15 = iClose(_Symbol, PERIOD_M15, 0);
        double sizeM15 = MathAbs(closeM15 - openM15) / (_Point * 10);
        
        // Deteccion de Latigazo por Velocidad (Movimiento fuerte en los ultimos 3 segundos via Ticks)
        MqlTick ticks[];
        ulong from_msc = (ulong)(TimeCurrent() - 3) * 1000; 
        int copied = CopyTicks(_Symbol, ticks, COPY_TICKS_ALL, from_msc, 0);
        bool esLatigazoVelocidad = false;
        double currentClose = iClose(_Symbol, PERIOD_M1, 0);
        double currentOpen = iOpen(_Symbol, PERIOD_M1, 0);
        
        if(copied > 0) {
            double price3SecAgo = ticks[0].bid;
            double bidNow = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            double latigazoPips = MathAbs(bidNow - price3SecAgo) / _Point / 10;
            if(latigazoPips >= 15.0) {
                esLatigazoVelocidad = true;
                currentClose = bidNow;
                currentOpen = price3SecAgo;
            }
        }
        
        // Mirar M1 (latigazo convencional)
        double sizeM1 = MathAbs(iClose(_Symbol, PERIOD_M1, 0) - iOpen(_Symbol, PERIOD_M1, 0)) / (_Point * 10);
        
        // Latigazo de 15 pips en 1 min, o 30 pips en 15 min, o 15 pips en 3 seg
        bool elefanteM15 = (sizeM15 >= VelaElefantePips);
        bool elefanteM1 = (sizeM1 >= 15.0); 
        
        if (elefanteM15 || elefanteM1 || esLatigazoVelocidad) {
            if (!esLatigazoVelocidad) {
                currentClose = iClose(_Symbol, PERIOD_M1, 0);
                currentOpen = (elefanteM1) ? iOpen(_Symbol, PERIOD_M1, 0) : openM15;
            }
            
            bool hayHedgeAlcista = false;
            bool hayHedgeBajista = false;
            for(int i=0; i<ArraySize(pos); i++) {
                if(PositionSelectByTicket(pos[i].ticket)) {
                    if(StringFind(PositionGetString(POSITION_COMMENT), "_HEDGE") >= 0) {
                        if(pos[i].t == POSITION_TYPE_BUY) hayHedgeAlcista = true;
                        if(pos[i].t == POSITION_TYPE_SELL) hayHedgeBajista = true;
                    }
                }
            }
            
            if (currentClose > currentOpen) {
                esVelaElefanteAlcista = true;
                condSell = false; // Prohibido vender contra vela elefante alcista
                // Subirse a la ola (Hedge) si tenemos ventas atrapadas
                if(ArraySize(pos) > 0 && pos[0].t == POSITION_TYPE_SELL && !hayHedgeAlcista) {
                    double volHedge = NormalizeDouble(volTotal * 1.5, 2); // 1.5x para ganar a favor de la ola 
                    if(volHedge > MaxLoteTotal) volHedge = MaxLoteTotal;
                    trade.Buy(volHedge, _Symbol, 0, 0, 0, TradeComment + "_HEDGE");
                    txtVoz = "ESCUDO HEDGE ALCISTA ACTIVADO!";
                    txtVeredicto = "LATIGAZO ALCISTA (HEDGE)";
                } else if(ArraySize(pos) == 0) {
                    trade.Buy(LotajeMinimo, _Symbol, 0, 0, 0, TradeComment + "_MOMENTUM");
                    txtVoz = "MOMENTUM ALCISTA (COMPRA)";
                    txtVeredicto = "MOMENTUM JUMP-IN";
                }
            } else {
                esVelaElefanteBajista = true;
                condBuy = false; // Prohibido comprar contra vela elefante bajista
                // Subirse a la ola (Hedge) si tenemos compras atrapadas
                if(ArraySize(pos) > 0 && pos[0].t == POSITION_TYPE_BUY && !hayHedgeBajista) {
                    double volHedge = NormalizeDouble(volTotal * 1.5, 2); // 1.5x para ganar a favor de la ola
                    if(volHedge > MaxLoteTotal) volHedge = MaxLoteTotal;
                    trade.Sell(volHedge, _Symbol, 0, 0, 0, TradeComment + "_HEDGE");
                    txtVoz = "ESCUDO HEDGE BAJISTA ACTIVADO!";
                    txtVeredicto = "LATIGAZO BAJISTA (HEDGE)";
                } else if(ArraySize(pos) == 0) {
                    trade.Sell(LotajeMinimo, _Symbol, 0, 0, 0, TradeComment + "_MOMENTUM");
                    txtVoz = "MOMENTUM BAJISTA (VENTA)";
                    txtVeredicto = "MOMENTUM JUMP-IN";
                }
            }
        }
...
// --- MODO LATERAL (Desconexi�n de H1/H4 si no hay tendencia) ---
    bool enModoLateral = (UsarModoMomentum && adxActual < MinADX_Lateral);
    
    if(CheckH4 && !enModoLateral && !esVelaElefanteAlcista && !esVelaElefanteBajista) { condBuy = condBuy && (bid > emaH4[0]); condSell = condSell && (bid < emaH4[0]); }
    if(CheckH1 && !enModoLateral && !esVelaElefanteAlcista && !esVelaElefanteBajista) { condBuy = condBuy && (bid > emaH1[0]); condSell = condSell && (bid < emaH1[0]); }
    ...
    if(UsarFiltroSR) {
        ...
        double distTecho = MathAbs(maxH1 - bid) / _Point / 10.0;
        double distSuelo = MathAbs(bid - minH1) / _Point / 10.0;

        if(condBuy && bid > maxH1 - MargenZonaPips * _Point * 10) { 
            condBuy = false; 
            txtVeredicto = StringFormat("TECHO DETECTADO (A %.1f pips) - COMPRA CANCELADA", distTecho);
            bloqueadoPorSR = true;
        }
        if(condSell && bid < minH1 + MargenZonaPips * _Point * 10) { 
            condSell = false; 
            txtVeredicto = StringFormat("SUELO DETECTADO (A %.1f pips) - VENTA CANCELADA", distSuelo);
            bloqueadoPorSR = true;
        }
    }
    ...
    if(ArraySize(pos) > 0) { 
        ...
        // Si estamos en emergencia, el flotante objetivo es el que asegura terminar el día con ProfitEmergenciaCents
    ...
    if(ArraySize(pos) > 0) { 
        ...
        txtVoz = StringFormat("VIGILANDO (%d/%d) CASCADA/SOS", ArraySize(pos), LimitePosicionesSOS + 1);
        ...
        if(UsarFiltroADX && adxActual < ADX_MinLevel) { txtVeredicto = "MERCADO LATERAL ADX ❌"; return; }
        ...
        if (direccion == 1) txtVoz = "BUSCANDO COMPRA...";
        else if (direccion == -1) txtVoz = "BUSCANDO VENTA...";
        else txtVoz = "ESPERANDO (MODO NEUTRAL)";

        bool emaOK = (direccion == 1 && bid > emaM5[0] && bid > emaM1[0] && bid > emaM1_9[0]) || (direccion == -1 && bid < emaM5[0] && bid < emaM1[0] && bid < emaM1_9[0]);

        if(!bloqueadoPorSR && !emaOK) txtVeredicto = "ESPERANDO CRUCE EMA (M5/M1/M1(9))...";
        else if(!bloqueadoPorSR && (pM5 == 0 || pM1 == 0)) txtVeredicto = "ESPERANDO FUERZA VELA (3 Pips min)";
        else if(!bloqueadoPorSR) {
...
void GestionarRefuerzoInteligente(double distSOS, double prExtrema, double distContActual) {
    ...
    } else txtVeredicto = "ESPERANDO CONFIRMACION LATIGAZO";
...
double CalcularGanadoUltraPreciso(int modo) {
    ...
    datetime start = 0; MqlDateTime dt; TimeCurrent(dt);
        ...
        if(modo == 0) { 
        dt.hour=0; dt.min=0; dt.sec=0; start = StructToTime(dt); 
        if(IgnorarHistorialPrevioHoy) start = MathMax(start, eaStartTime);
    }
...
void CrearInterfazMaster() { 
    ...
    CrearLabel("MAIKO_T", x+10, y+10, "MAIKO HYBRID | CENT PRO v13.92", ColorMain, 11, 10001); 
