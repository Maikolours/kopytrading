...
input string   TradeComment               = "MAIKO_PRO_GOLD_DEMO"; // 📝 Comentario para Órdenes (Trade Comment)
...
const demoMq5Main = path.join(baseProject, "MAIKO_PRO_GOLD_DEMO.mq5");
const demoMq5Sub = path.join(targetSubfolder, "MAIKO_PRO_GOLD_DEMO.mq5");

fs.writeFileSync(demoMq5Main, exactDemoCode, 'utf8');
fs.writeFileSync(demoMq5Sub, exactDemoCode, 'utf8');
...
if (fs.existsSync(editorPath)) {
    try {
        console.log("Compilando MAIKO_PRO_GOLD_DEMO.mq5 desde el codigo exacto del usuario...");
        execSync(`"${editorPath}" /compile:"${demoMq5Main}" /log`);
        execSync(`"${editorPath}" /compile:"${demoMq5Sub}" /log`);
        console.log("✓ Compilacion DEMO exitosa 100%.");
    } catch(e) {
...
const demoEx5Main = path.join(baseProject, "MAIKO_PRO_GOLD_DEMO.ex5");
...
if (fs.existsSync(basePath)) {
    ...
    folders.forEach((f) => {
        ...
        if (fs.existsSync(demoEx5Main)) {
            fs.copyFileSync(demoEx5Main, path.join(expertsDir1, "MAIKO_PRO_GOLD_DEMO.ex5"));
            fs.copyFileSync(demoEx5Main, path.join(expertsDir2, "MAIKO_PRO_GOLD_DEMO.ex5"));
        }
        fs.copyFileSync(demoMq5Main, path.join(expertsDir1, "MAIKO_PRO_GOLD_DEMO.mq5"));
        fs.copyFileSync(demoMq5Main, path.join(expertsDir2, "MAIKO_PRO_GOLD_DEMO.mq5"));
    });
    console.log("✓ Desplegado MAIKO_PRO_GOLD_DEMO 100% idéntico en todos los terminales MT5.");
}
