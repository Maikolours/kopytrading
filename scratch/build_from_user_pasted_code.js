...
// Tomar exactamente el código pegado por el usuario
let code = `//+------------------------------------------------------------------+

//|            MAIKO PRO GOLD M1 SHIELD v11.32  CENT                     |
...
#property copyright "MAIKO PRO GOLD SHIELD CENT"
...
const bool     EsCuentaCent               = true;        // CUENTA CENT ACTIVADA (Hardcoded)
...
input double   DistanciaCascadaPips       = 0.0;         // 🌊 Distancia disparo Cascada M1 (0.0 = DESACTIVADA POR SEGURIDAD)

input group "━━━━━━ 🏛️ FILTRO DE TECHOS Y SUELOS SEMANALES (W1) ━━━━━━"
...
// --- FILTROS DE TECHOS Y SUELOS (SOPORTES Y RESISTENCIAS) ---
...
// --- OPERATIVA Y LOTES (CALIBRADOS PARA CUENTA CENT) ---
input group "------- 🏛️ FILTRO DE TECHOS Y SUELOS -------"
input double   LoteAtaque                 = 0.10;        // 🚀 Volumen Entrada Inicial (Ataque Cent)
...
input double   MaxLoteTotal               = 5.0;         // 🚫 Lote Máximo Acumulado Permitido (Cent)
input double   MaxLoteIndividual          = 0.15;        // 🚫 Volumen Máximo por Operación SOS (Cent)

// --- COBRAR BENEFICIOS (TAKE PROFIT EN CENTAVOS) ---
input group "------- 📉 TENDENCIA Y DIRECCION -------"
input double   ProfitCosechaIndividual    = 25.0;        // 💵 Beneficio Cierre SOS Individual (Centavos = $0.25)
input double   TargetDiario               = 500.0;       // 🎯 Meta de Beneficio Diario (Centavos = $5.00)
input double   ProfitNetoFlush            = 50.0;        // 💵 Beneficio Cierre Total Cesta (Centavos = $0.50)
input double   ProfitBreakEven            = 5.0;         // 🛡️ Beneficio Mínimo Break Even Cesta (Centavos)

// --- HORARIOS OPERATIVOS SEGUROS ---
...
// --- PROTECCIONES Y SEGURIDAD BLINDADAS ---
input group "------- 📏 DISTANCIAS Y CASCADA -------"
input int      LimitePosicionesSOS        = 2;           // 📉 Límite Máximo Posiciones SOS
input bool     UsarStopLossPorcentaje     = true;        // 🛡️ Activar Stop Loss por % Cuenta (BLINDADO)
input double   PorcentajeStopLoss         = 10.0;        // 📉 Porcentaje de Pérdida Máxima...
input bool     UsarPausaTrasStopLoss      = true;        // ⏸️ Pausar Bot tras un Stop Loss (BLINDADO)
input int      MinutosPausaTrasStopLoss   = 30;          // ⏳ Minutos de Pausa tras Stop Loss

// --- HUD ---
input group "------- 💰 COBRAR BENEFICIOS (TP) -------"
string         HUD_Branding               = "MAIKO PRO SHIELD CENT";
...
input string   TradeComment               = "MAIKO_SHIELD_CENT";        // 🏷️ Comentario para Órdenes (Trade Comment)
input int      ExpertMagic                = 111335;                   // 🔢 Magic Number del Bot Cent
...
double CalcularGanadoHoy() { 
    ...
    for(int i=0; i<valid_deals; i++) {
        ...
        if(d_magic == ExpertMagic) total += deals[i].profit;
...
bool ValidarTechosSuelos(string decision) {
    ...
    if(UsarFiltroTechosSuelosW1) {
        int highest_idx = iHighest(_Symbol, PERIOD_W1, MODE_HIGH, 52, start_bar);
        int lowest_idx = iLowest(_Symbol, PERIOD_W1, MODE_LOW, 52, start_bar);
        if(highest_idx >= 0 && lowest_idx >= 0) {
            ...
            if(decision == "BUY") {
                ...
                if(dist_to_ceiling > 0 && dist_to_ceiling <= DistanciaTechoSueloPipsW1) {
                ...
                if(dist_to_floor > 0 && dist_to_floor <= DistanciaTechoSueloPipsW1) {
...
if (fs.existsSync(editorPath)) {
    try {
        console.log("Compilando MAIKO_PRO_GOLD_CENT.mq5 desde el codigo exacto del usuario...");
        execSync(`"${editorPath}" /compile:"${centMq5Main}" /log`);
        execSync(`"${editorPath}" /compile:"${centMq5Sub}" /log`);
        console.log("✓ Compilacion exitosa 100%.");
    } catch(e) {
...
if (fs.existsSync(basePath)) {
    ...
    console.log("✓ Desplegado MAIKO_PRO_GOLD_CENT 100% exacto basado en el codigo del usuario.");
}
