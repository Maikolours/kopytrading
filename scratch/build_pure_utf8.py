...
original_file = r"C:\proyectos\APP KOPYTRADING\Agosto_2026_Activos\00_OFICIALES_SEPTIEMBRE\MAIKO_PRO_GOLD_DEMO.mq5"

# The original file is UTF-8
with open(original_file, 'r', encoding='utf-8') as f:
    code = f.read()

# Strip any BOM if it accidentally got one
if code.startswith('\ufeff'):
    code = code[1:]
...
# 2. Inputs
new_inputs = """input bool     OperarViernesNoche         = false;       // 🌃 Permitir Operaciones Viernes Tarde (17:00+)
...
code = re.sub(r'void AgregarIndicadoresVisuales\(\)\s*\{.*?(?=\n\w|\n\n\w)', new_visual, code, flags=re.DOTALL)

# 5. OnInit Handles
on_init_handles = """hRSI_v = iRSI(_Symbol, _Period, 14, PRICE_CLOSE);
    hATR_v = iATR(_Symbol, _Period, 14);
    hBands_v = iBands(_Symbol, _Period, 20, 0, 2.0, PRICE_CLOSE);"""
code = re.sub(r'hRSI_v = iRSI\(_Symbol, _Period, 14, PRICE_CLOSE\);', on_init_handles, code)

# 6. OnDeinit Release
on_deinit_release = """if(hRSI_v != INVALID_HANDLE) IndicatorRelease(hRSI_v);
    if(hATR_v != INVALID_HANDLE) IndicatorRelease(hATR_v);
    if(hBands_v != INVALID_HANDLE) IndicatorRelease(hBands_v);"""
code = re.sub(r'if\(hRSI_v != INVALID_HANDLE\)\s*IndicatorRelease\(hRSI_v\);', on_deinit_release, code)
...
# JSON Escapes
code = code.replace('StringReplace(narrative, """, "\'");', r'StringReplace(narrative, "\"", "\'");')
...
code = code.replace('StringFind(response, ""armed":true")', r'StringFind(response, "\"armed\":true")')
...
"""
code = code.replace("void OnTick() {", m5_func + "void OnTick() {")

entry_orig = 'if(!enFaseAnalisis) { enFaseAnalisis = true; proximoAtaque = serverTime + 60; txtVoz = "SCHOLAR: Buscando..."; ActualizarInterfazMaster(); }'
entry_rep = """// Comprobar filtro anti-turbulencia M5 antes de entrar
...
code = code.replace(entry_orig, entry_rep)
...
# Write exactly as pure UTF-8 (no BOM!)
