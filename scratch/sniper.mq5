...
input string   MiLicencia                 = "";// Licencia o Correo de Usuario
...
input double   MaxRsiCompra               = 70.0;// RSI Mximo para Compras (Filtro Techos)
input double   MinRsiVenta                = 30.0;// RSI Mnimo para Ventas (Filtro Suelos)
...
input group "=== FILTROS RSI ==="
input bool             UsarFiltroTechosSuelos     = true;// Activar Filtro Techos y Suelos M15 (S/R)
input ENUM_TIMEFRAMES  TimeframeTechosSuelos      = PERIOD_M15;// Temporalidad para Techos/Suelos M15
input int              PeriodoTechosSuelos        = 24;// Perodo de Velas M15 a Analizar
input double           DistanciaTechoSueloPips    = 15.0;// Distancia Mnima M15 para Bloquear (Pips)

// --- FILTROS ADICIONALES MULTI-TEMPORALIDAD (H1 y H4) ---
input bool             UsarFiltroTechosSuelosH1   = true;// Activar Filtro S/R en H1
input int              PeriodoTechosSuelosH1      = 24;// Perodo H1 a Analizar (Velas)
input double           DistanciaTechoSueloPipsH1  = 30.0;// Distancia Mnima H1 (Pips)

input bool             UsarFiltroTechosSuelosH4   = true;// Activar Filtro S/R en H4
input int              PeriodoTechosSuelosH4      = 24;// Perodo H4 a Analizar (Velas)
input double           DistanciaTechoSueloPipsH4  = 50.0;// Distancia Mnima H4 (Pips)
...
input double           MinPorcentajeMechaM15      = 40.0;// % Mnimo Mecha Reversa (40.0 = 40%)

// --- CONFIRMACION DE RUPTURA ---
input bool             UsarConfirmacionRuptura    = true;// Confirmar Ruptura de S/R con Vela Cerrada
input ENUM_TIMEFRAMES  TimeframeConfirmacion      = PERIOD_M5;// Temporalidad de Confirmacin (M5/M15)

// --- OPERATIVA Y LOTES ---
input group "=== TENDENCIA Y LOTAJE M15 ==="
input double   LoteAtaque                 = 0.01;// Volumen Entrada Inicial (Ataque)
input double   MultiplicadorRefuerzo      = 1.5;// Multiplicador Lote de Rescate (SOS)
input double   DistanciaRefuerzoPips      = 80.0;// Distancia Mnima para Abrir SOS (Pips)
input double   MaxLoteTotal               = 0.50;// Lote Mximo Acumulado Permitido
input double   MaxLoteIndividual          = 0.02;// Volumen Mximo por Operacin SOS

// --- COBRAR BENEFICIOS (TAKE PROFIT) ---
input group "=== TENDENCIA H1 ==="
...
input double   ProfitBreakEven            = 0.50;// Beneficio Mnimo Break Even Cesta ($)

// --- HORARIOS OPERATIVOS ---
input group "=== TENDENCIA H4 ==="
...
input group "=== FILTRO AGOTAMIENTO ==="
input int      LimitePosicionesSOS        = 2;// Lmite Mximo Posiciones SOS
input bool     UsarStopLossPorcentaje     = false;// Activar Stop Loss por % Cuenta
input double   PorcentajeStopLoss         = 10.0;// Porcentaje de Prdida Mxima (%)
...
// --- HUD ---
input group "=== CONFIRMACION RUPTURA ==="
string         HUD_Branding               = "MAIKO v11.30 | CLIENT REAL";
...
input int      HUD_X                      = 15;// Posicin X en Pantalla (Pixeles)
input int      PosY_HUD                   = 25;// Posicin Y en Pantalla (Pixeles)
bool           ShowW1                     = true;
bool           ShowD1                     = true;
bool           ShowH4                     = true;
bool           ShowH1                     = true;
...
// --- COMENTARIOS ---
input group "=== OPERATIVA Y GESTION LOTES ==="
input string   TradeComment               = "MAIKO_REAL";// Comentario para rdenes (Trade Comment)
