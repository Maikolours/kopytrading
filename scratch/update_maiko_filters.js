...
const newInputs = `// --- FILTRO DE TECHOS Y SUELOS (SOPORTES Y RESISTENCIAS) ---
...
files.forEach(file => {
    ...
    // 1. Replace S/R inputs
    content = content.replace(/\/\/ --- FILTRO DE TECHOS Y SUELOS \(SOPORTES Y RESISTENCIAS\) ---[\s\S]*?DistanciaTechoSueloPips\s*=\s*15\.0;[^\r\n]*/, newInputs);

    // 2. Replace ProfitCosechaIndividual if it was 1.0
    content = content.replace(/input double\s+ProfitCosechaIndividual\s*=\s*1\.0;/g, 'input double   ProfitCosechaIndividual    = 0.75;        // 💵 Beneficio Cierre SOS Individual ($)');
