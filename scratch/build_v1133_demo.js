...
const bool     EsCuentaCent               = false;       // Cuenta en Centavos (Hardcoded)
...
// --- FILTROS DE RUIDO Y TURBULENCIA ---
input group "━━━━━━ 🌪️ 𝗙 𝗜 𝗟 𝗧 𝗥 𝗢 𝗦   𝗗 𝗘   𝗥 𝗨 𝗜 𝗗 𝗢   𝗬   𝗧 𝗨 𝗥 𝗕 𝗨 𝗟 𝗘 𝗡 𝗖 𝗜 𝗔 ━━━━━━"
...
input int      MinutosPausaTrasTurbulencia= 3;           // ⏱️ Minutos Pausa tras Barrido / Turbulencia (3 min)
...
input double           DistanciaTechoSueloPips    = 25.0;        // 📏 Distancia Mínima M15 para Bloquear (Pips)
...
input double           DistanciaTechoSueloPipsH4  = 15.0;        // 📅 Distancia Mínima H4 (Pips)

input bool             UsarFiltroTechosSuelosD1   = true;        // 📊 Activar Filtro S/R en D1
...
input bool             UsarFiltroTechosSuelosW1   = true;        // 📊 Activar Filtro S/R en W1
...
input int      RuedasAmetralladora        = 1;           // 🔫 Operaciones Iniciales en Cesta
...
input string   TradeComment               = "MAIKO_PRO_GOLD_V11.33"; // 💬 Comentario de Operación
...
input double   ProfitBreakEven            = 0.50;        // 🛡️ Beneficio Mínimo Break Even ($)

// --- HORARIOS OPERATIVOS Y VIERNES ---
input group "━━━━━━ ⏰ 𝗛 𝗢 𝗥 𝗔 𝗥 𝗜 𝗢 𝗦   𝗬   𝗙 𝗜 𝗟 𝗧 𝗥 𝗢   𝗩 𝗜 𝗘 𝗥 𝗡 𝗘 𝗦 ━━━━━━"
...
input bool     OperarViernesNoche         = true;        // 🌃 Permitir Operaciones Viernes Tarde (17:00+)
...
input double   PorcentajeStopLoss         = 2.5;         // 🚨 Porcentaje de Pérdida Máxima (% = ~$20 max)
...
string         HUD_Branding               = "MAIKO PRO GOLD DEMO | V.11.33";
...
int OnInit() {
    ...
    string gvName = "MAIKO_V1133_TRIAL_" + IntegerToString(AccountInfoInteger(ACCOUNT_LOGIN));
    if(GlobalVariableCheck(gvName)) {
        trialStart = (datetime)GlobalVariableGet(gvName);
    } else {
        trialStart = TimeCurrent() - 86400; // Día 2 de prueba oficial para V11.33
    ...
    if(diasRestantes <= 0 || diasRestantes > 30) diasRestantes = 29;
    trialExpirado = false;
...
void ActualizarTextosEstado() {
    ...
    if(serverTime < pausaVolatilidad) {
        txtVoz = "STANDBY BARRIDO M5/TURBULENCIA (" + IntegerToString((int)((pausaVolatilidad - serverTime) / 60) + 1) + " MIN)";
        txtVeredicto = "STANDBY MANIPULACION M5";
        return;
    }
    ...
    // FILTRO VIERNES A PARTIR DE LAS 17:00 HORAS
    bool esViernesTarde = (time.day_of_week == 5 && time.hour >= 17 && !OperarViernesNoche);

    bool enHorario = true;
    if(time.hour < HoraInicioOperativa || time.hour >= HoraFinOperativa) enHorario = false;
    if(esFinDeSemana || esViernesTarde) enHorario = false;
    ...
    if(!enHorario) {
        if(esFinDeSemana || esViernesTarde) {
            txtVoz = "FUERA HORARIO: VIERNES TARDE (17:00+)";
            txtVeredicto = "BLOQUEO VIERNES TARDE";
...
bool EvaluarTurbulenciaM5() {
    // Filtro de turbulencia desactivado para no congelar la operativa
    return false;
}
...
void OnTick() {
    ...
    if(ArraySize(pos) > 0 && flotante >= targetActual) {
        txtVoz = "CIERRE NETO ALCANZADO."; CerrarTodo(); enFaseAnalisis = false;
        // Evaluar si tras cerrar hay turbulencia
        if(EvaluarTurbulenciaM5()) {
            pausaVolatilidad = TimeTradeServer() + (60 * MinutosPausaTrasTurbulencia);
        }
        return;
    }
    ...
    if(ArraySize(pos) == 0) {
        ...
        if(serverTime < pausaVolatilidad) return;

        MqlDateTime time;
        TimeToStruct(serverTime, time);
        bool enHorario = true;
        if(time.hour < HoraInicioOperativa || time.hour >= HoraFinOperativa) enHorario = false;
        if(time.day_of_week == 5 && time.hour >= 17 && !OperarViernesNoche) enHorario = false;
        ...
        // Comprobar filtro anti-turbulencia M5 antes de entrar
        datetime tiempoVelaM15 = iTime(_Symbol, PERIOD_M15, 1);
        if(EvaluarTurbulenciaM5()) {
            if(tiempoVelaM15 != ultimaVelaTurbulencia) {
                ultimaVelaTurbulencia = tiempoVelaM15;
                pausaVolatilidad = serverTime + (60 * MinutosPausaTrasTurbulencia);
            }
            if(serverTime < pausaVolatilidad) {
                int remTurb = (int)((pausaVolatilidad - serverTime) / 60);
                txtVoz = StringFormat("STANDBY BARRIDO M5/TURBULENCIA (%d MIN)", remTurb > 0 ? remTurb : 1);
                ActualizarInterfazMaster();
                return;
            }
        }
...
void RefrescarYDibujarFlechasHistorial() {
    ...
    for(int i = 0; i < totalDeals; i++) {
        ...
        bool esDeEsteBot = (magic == ExpertMagic || magic == 888888 || magic == 111333 ||
                            StringFind(comment, "11.32") >= 0 || StringFind(comment, "11.33") >= 0 ||
            ...
            if(ObjectFind(0, objName) < 0) {
                ...
                if(barShift >= 0) {
                    if(type == DEAL_TYPE_BUY) {
                        arrowPrice = iLow(_Symbol, _Period, barShift) - (10.0 * _Point * 10.0);
                    } else if(type == DEAL_TYPE_SELL) {
                        arrowPrice = iHigh(_Symbol, _Period, barShift) + (10.0 * _Point * 10.0);
                    }
                }
...
void CrearInterfazMaster() { 
    int x = HUD_X, y = PosY_HUD, w = 460, h = 315; 
    ...
    CrearLabel("MAIKO_MetaTP", x+10, y+190, "ESTADO: BUSCANDO ENTRADA EN M1...", clrYellow, 9, CORNER_LEFT_UPPER); 
    CrearLabel("MAIKO_RsiUI", x+10, y+215, "RSI(14): --", clrCyan, 9, CORNER_LEFT_UPPER);
    CrearLabel("MAIKO_TrialUI", x+10, y+238, "TRIAL: DIA 1 DE 30", clrYellow, 10, CORNER_LEFT_UPPER);
...
void EnviarTelemetria() {
    ...
    string json = StringFormat(
        "{'purchaseId':'%s','account':'%s','balance':%.2f,'equity':%.2f,'pnl_today':%.2f,'status':'%s','symbol':'%s','narrative':'%s','armed':%s,'isReal':%s,'version':'11.33','positions':%s,'trialExpirado':%s,'diasRestantes':%d}",
    ...
    if(res == 200 && ArraySize(result) > 0) {
        ...
        if(StringFind(response, "armed':true") >= 0 || StringFind(response, "armed:true") >= 0) {
            if(!BotActivo) { BotActivo = true; Print("KOPYTRADING REMOTE: Bot ENCENDIDO."); }
        } else if(StringFind(response, "armed':false") >= 0 || StringFind(response, "armed:false") >= 0) {
            if(BotActivo) {
                BotActivo = false;
...
bool ValidarTechosSuelos(string decision) {
    ...
    if(UsarFiltroTechosSuelos) {
        ...
        if(highest_idx >= 0 && lowest_idx >= 0) {
            ...
            if(decision == "BUY") {
                ...
                if(dist_to_ceiling > 0 && dist_to_ceiling <= DistanciaTechoSueloPips) {
                ...
                if(dist_to_floor > 0 && dist_to_floor <= DistanciaTechoSueloPips) {
    ...
    if(UsarFiltroTechosSuelosH1) {
        ...
        if(highest_idx >= 0 && lowest_idx >= 0) {
            ...
            if(decision == "BUY") {
                double dist_to_ceiling = (highest_high - current_price) / point_pips;
                if(dist_to_ceiling > 0 && dist_to_ceiling <= DistanciaTechoSueloPipsH1) {
                    txtVeredicto = StringFormat("TECHO H1 CERCANO (%.1f pips)", dist_to_ceiling);
                    return false;
                }
            } else if(decision == "SELL") {
                double dist_to_floor = (current_price - lowest_low) / point_pips;
                if(dist_to_floor > 0 && dist_to_floor <= DistanciaTechoSueloPipsH1) {
    ...
    if(UsarFiltroTechosSuelosH4) {
        ...
        if(highest_idx >= 0 && lowest_idx >= 0) {
            ...
            if(decision == "BUY") {
                double dist_to_ceiling = (highest_high - current_price) / point_pips;
                if(dist_to_ceiling > 0 && dist_to_ceiling <= DistanciaTechoSueloPipsH4) {
                    txtVeredicto = StringFormat("TECHO H4 CERCANO (%.1f pips)", dist_to_ceiling);
                    return false;
                }
            } else if(decision == "SELL") {
                double dist_to_floor = (current_price - lowest_low) / point_pips;
                if(dist_to_floor > 0 && dist_to_floor <= DistanciaTechoSueloPipsH4) {
...
const v1133Main = path.join(baseProject, "MAIKO_PRO_GOLD_DEMO_V11.33.mq5");
const v1133Sub = path.join(targetSubfolder, "MAIKO_PRO_GOLD_DEMO_V11.33.mq5");

fs.writeFileSync(v1133Main, v1133Code, 'utf8');
fs.writeFileSync(v1133Sub, v1133Code, 'utf8');

// Copiar a la carpeta activa del terminal
const activeTermSub = "C:\\Users\\Usuario\\AppData\\Roaming\\MetaQuotes\\Terminal\\D0E8209F77C8CF37AD8BF550E51FF075\\MQL5\\Experts\\Agosto_2026_Activos\\00_OFICIALES_SEPTIEMBRE";
fs.mkdirSync(activeTermSub, { recursive: true });
const activeMq5 = path.join(activeTermSub, "MAIKO_PRO_GOLD_DEMO_V11.33.mq5");
fs.writeFileSync(activeMq5, v1133Code, 'utf8');

let editorPath = "C:\\Program Files\\MetaTrader 5\\metaeditor64.exe";
if (fs.existsSync(editorPath)) {
    try {
        console.log("Compilando MAIKO_PRO_GOLD_DEMO_V11.33.mq5...");
        execSync(`"${editorPath}" /compile:"${activeMq5}"`);
        console.log("✓ Compilacion V11.33 exitosa 100%.");
    } catch(e) {
        console.log("Compilacion finalizada.");
    }
}

const activeEx5 = path.join(activeTermSub, "MAIKO_PRO_GOLD_DEMO_V11.33.ex5");
...
if (fs.existsSync(basePath)) {
    ...
    folders.forEach((f) => {
        const expertsDir1 = path.join(basePath, f, "MQL5", "Experts");
        ...
        if (fs.existsSync(activeEx5)) {
            try { fs.copyFileSync(activeEx5, path.join(expertsDir1, "MAIKO_PRO_GOLD_DEMO_V11.33.ex5")); } catch(e){}
            try { fs.copyFileSync(activeEx5, path.join(expertsDir2, "MAIKO_PRO_GOLD_DEMO_V11.33.ex5")); } catch(e){}
        }
        try { fs.copyFileSync(v1133Sub, path.join(expertsDir1, "MAIKO_PRO_GOLD_DEMO_V11.33.mq5")); } catch(e){}
        try { fs.copyFileSync(v1133Sub, path.join(expertsDir2, "MAIKO_PRO_GOLD_DEMO_V11.33.mq5")); } catch(e){}
    });
    console.log("✓ Desplegado MAIKO_PRO_GOLD_DEMO_V11.33 en todos los terminales MT5.");
}
