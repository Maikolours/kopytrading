...
async function main() {
    console.log('--- Starting Bot Catalog Migration ---');
    ...
    // 3. Upsert bots based on instrument (unique enough for our needs)
    for (const data of targetBots) {
        // Try to find an existing bot by instrument or name to reuse its ID if possible
